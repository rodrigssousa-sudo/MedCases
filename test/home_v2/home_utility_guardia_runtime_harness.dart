...
Future<void> verifyUtilityRuntime(WidgetTester tester,
  ...
  expect(tester.widgetList<Text>(find.byType(Text)).map((t) => t.data), labels);
...
Future<void> verifyGuardiaRuntime(WidgetTester tester,
    {bool dark = false, bool isEs = false}) async {
  var taps = 0;
  await tester.pumpWidget(MaterialApp(
      home: Scaffold(
    ...
    isEs: isEs,
    child:
        TextButton(onPressed: () => taps++, child: const Text('Open patient')),
  ))));
  await tester.pumpAndSettle();
