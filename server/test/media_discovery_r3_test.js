...
for(const budget of [null,1,5000])test('background production segment handler central classifier budget='+budget,async()=>{
 ...
 const baseCollection=db.collection.bind(db);db.collection=name=>({doc:id=>{const ref=baseCollection(name).doc(id);ref.collection=child=>db.collection(name+'/'+id+'/'+child);return ref;}});
 ...
 assert.equal(calls,budget===5000?1:0,JSON.stringify(out.body));assert.equal(out.code,budget===5000?200:502);assert.ok(bytes.every(x=>x===0));
});
