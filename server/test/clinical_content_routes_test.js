...
test('endpoint fails closed until publication root configured',async()=>{assert.equal((await run()).res.code,500);});
...
// Validate the public contract over real Express HTTP, not only a response stub.
test('availability failures retain enumerated JSON with HTTP 500', async () => {
  const express = require('express');
  const root = await fs.mkdtemp(path.join(os.tmpdir(), 'rk-http-contract-'));
  try {
    for (const [configuredRoot, expected] of [[undefined, 'NOT_CONFIGURED'], [root, 'CONTENT_UNAVAILABLE']]) {
      const app = express();
      registerClinicalContentRoutes({app, root: configuredRoot,
        authenticate: (req, res, next) => { req.auth = {uid: 'TECHNICAL_UID'}; next(); },
      ...
      try {
        const response = await fetch(`http://127.0.0.1:${server.address().port}/api/clinical-content/manifest.json`);
        assert.equal(response.status, 500);
        assert.match(response.headers.get('content-type'), /^application\/json/);
        assert.equal(response.headers.get('cache-control'), 'private, no-store');
        assert.equal(await response.text(), JSON.stringify({error: expected}));
