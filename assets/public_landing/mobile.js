(()=>{
const section=document.getElementById('mobile'),stage=section.querySelector('.phone-stage'),phones=[...section.querySelectorAll('.showcase-phone')],buttons=[...section.querySelectorAll('[data-mobile-slide]')],pause=section.querySelector('.mobile-pause'),full=section.querySelector('.mobile-full'),motion=matchMedia('(prefers-reduced-motion: reduce)'),count=phones.length;
let index=0,paused=motion.matches,visible=false,timer=null,start=null;
function label(){const es=document.documentElement.lang==='es';pause.textContent=paused?(es?'Reanudar rotación':'Retomar rotação'):(es?'Pausar rotación':'Pausar rotação');pause.setAttribute('aria-label',paused?(es?'Reanudar cambio de pantallas':'Retomar troca de telas'):(es?'Pausar cambio de pantallas':'Pausar troca de telas'));pause.setAttribute('aria-pressed',String(paused));phones.forEach((phone,i)=>phone.querySelector('img').alt='MedCases Pro — '+buttons[i].textContent);}
