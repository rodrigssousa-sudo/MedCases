...
class SoapNotifier extends ChangeNotifier {
  // MEDCASES_PACIENTES_FINAL_SOAP_SOURCE_DRIVEN_V2
  ...
  void applyAiDraft(SoapDraftResult draft) {
    ...
    final newSubjetivo = s.copyWith(
      notePasaNoche: draft.notePasaNoche?.isNotEmpty == true
          ? draft.notePasaNoche
          : (s.notePasaNoche.isEmpty ? '' : s.notePasaNoche),
      ...
      notasLibres: draft.notasLibresSubjetivo?.isNotEmpty == true
          ? draft.notasLibresSubjetivo
          : (s.notasLibres.isEmpty ? '' : s.notasLibres),
    );

    // O — Signos vitales
    final sv = o.signosVitales;
    final newSv = sv.copyWith(
      pa: draft.pa?.isNotEmpty == true
          ? draft.pa
          : (sv.pa.isEmpty ? '' : sv.pa),
      fc: draft.fc?.isNotEmpty == true
          ? draft.fc
          : (sv.fc.isEmpty ? '' : sv.fc),
      fr: draft.fr?.isNotEmpty == true
          ? draft.fr
          : (sv.fr.isEmpty ? '' : sv.fr),
      satO2: draft.satO2?.isNotEmpty == true
          ? draft.satO2
          : (sv.satO2.isEmpty ? '' : sv.satO2),
      temperatura: draft.temperatura?.isNotEmpty == true
          ? draft.temperatura
          : (sv.temperatura.isEmpty ? '' : sv.temperatura),
    );
    ...
    final newEf = ef.copyWith(
      estadoGeneral: draft.estadoGeneral?.isNotEmpty == true
          ? draft.estadoGeneral
          : (ef.estadoGeneral.isEmpty ? '' : ef.estadoGeneral),
      acv: draft.acv?.isNotEmpty == true
          ? draft.acv
          : (ef.acv.isEmpty ? '' : ef.acv),
      ar: draft.ar?.isNotEmpty == true
          ? draft.ar
          : (ef.ar.isEmpty ? '' : ef.ar),
      abdomen: draft.abdomen?.isNotEmpty == true
          ? draft.abdomen
          : (ef.abdomen.isEmpty ? '' : ef.abdomen),
      extremidades: draft.extremidades?.isNotEmpty == true
          ? draft.extremidades
          : (ef.extremidades.isEmpty ? '' : ef.extremidades),
    );
    ...
    final newEx = ex.copyWith(
      laboratorio: draft.laboratorio?.isNotEmpty == true
          ? draft.laboratorio
          : (ex.laboratorio.isEmpty ? '' : ex.laboratorio),
      imagenes: draft.imagenes?.isNotEmpty == true
          ? draft.imagenes
          : (ex.imagenes.isEmpty ? '' : ex.imagenes),
      culturas: draft.culturas?.isNotEmpty == true
          ? draft.culturas
          : (ex.culturas.isEmpty ? '' : ex.culturas),
      ecg: draft.ecg?.isNotEmpty == true
          ? draft.ecg
          : (ex.ecg.isEmpty ? '' : ex.ecg),
    );
    ...
    final newObjetivo = o.copyWith(
      ...
      tratamientoActual: draft.tratamientoActual?.isNotEmpty == true
          ? draft.tratamientoActual
          : (o.tratamientoActual.isEmpty ? '' : o.tratamientoActual),
    );
    ...
    final newEvaluacion = a.copyWith(
      ...
      notasEvaluacion: draft.notasEvaluacion?.isNotEmpty == true
          ? draft.notasEvaluacion
          : (a.notasEvaluacion.isEmpty ? '' : a.notasEvaluacion),
    );

    // P — Plan
    final newPlan = p.copyWith(
      planTerapeutico: draft.planTerapeutico?.isNotEmpty == true
          ? draft.planTerapeutico
          : (p.planTerapeutico.isEmpty ? '' : p.planTerapeutico),
      criteriosAlta: draft.criteriosAlta?.isNotEmpty == true
          ? draft.criteriosAlta
          : (p.criteriosAlta.isEmpty ? '' : p.criteriosAlta),
    );
  ...
  String? _mapAiAlimentacion(String? v) {
    ...
    if (lower.contains('bien') ||
        lower.contains('boa') ||
        lower.contains('buen')) return v;
...
class SoapSectionWidgetState extends State<SoapSectionWidget> {
  ...
  Future<void> _executeCopy(_CopyModel model) async {
    final ev = _notifier
        .evolucion; // fonte viva: notifier (não _draftEvolucion estático)
    final isEs = widget.lang == 'es';
    ...
    debugPrint(
        '[COPY_211] autorFinal=$autorFinal (ev=$evAutor / widget=$widgetAutor)');
...
class _SoapAccordion extends StatelessWidget {
  // MEDCASES_PACIENTES_SOAP_FINAL_FLAT_V4
  ...
  @override
  Widget build(BuildContext context) {
    final accent = theme.soapTagFg(section);
    final neutralBorder =
        theme.border.withOpacity(dark ? 0.62 : 0.58);
    final quietChevron =
        theme.border.withOpacity(dark ? 0.18 : 0.24);
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: accent.withOpacity(dark ? 0.13 : 0.09),
...
String soapCompletoString(
  ...
  buf.writeln(sep);
  buf.writeln(
      '📋 MEDCASES PRO - ${isEs ? 'PRONTUARIO COMPLETO' : 'PRONTUÁRIO COMPLETO'}');
  buf.writeln(sep);
  ...
  if (p != null) {
    ...
    final idade =
        p.idade.isNotEmpty ? ' • ${p.idade} ${isEs ? 'años' : 'anos'}' : '';
    final svc = isEs ? 'Servicio' : 'Serviço';
    buf.writeln(
        '👤 $nome$idade | 🛏️ ${isEs ? 'Cama' : 'Leito'}: $cama | HC: ______');
    buf.writeln(
        '📆 ${isEs ? 'Día de internación' : 'Dia de internação'}: ${p.diaInternacao} | 🩺 $svc: ______');
    if (p.sexo.isNotEmpty)
      buf.writeln('⚧  ${isEs ? 'Sexo' : 'Sexo'}: ${p.sexo}');
    if (p.diagnostico.isNotEmpty) {
      buf.writeln(
          '🩺 ${isEs ? 'Diagnóstico principal' : 'Diagnóstico principal'}: ${p.diagnostico}');
    }
  } else {
  ...
  if (s.fiebre) {
    hasS = true;
    buf.writeln('• ${isEs ? 'Fiebre' : 'Febre'}: presente');
  }
  if (s.disnea) {
    hasS = true;
    buf.writeln('• ${isEs ? 'Disnea' : 'Dispneia'}: presente');
  }
  if (s.nauseas) {
    hasS = true;
    buf.writeln('• ${isEs ? 'Náuseas' : 'Náuseas'}: presente');
  }
  if (s.tos) {
    hasS = true;
    buf.writeln('• ${isEs ? 'Tos' : 'Tosse'}: presente');
  }
  if (s.suenoRestado) {
    hasS = true;
    buf.writeln('• ${isEs ? 'Sueño alterado' : 'Sono alterado'}: presente');
  }
  ...
  if (ef.estadoGeneral.isNotEmpty) {
    hasO = true;
    buf.writeln(
        '• ${isEs ? 'Estado General' : 'Estado Geral'}: ${ef.estadoGeneral}');
  }
  ...
  if (ef.extremidades.isNotEmpty) {
    hasO = true;
    buf.writeln(
        '• ${isEs ? 'Extremidades' : 'Extremidades'}: ${ef.extremidades}');
  }
  ...
  if (a.estado != null) {
    hasA = true;
    buf.writeln(
        '• ${isEs ? 'Status Clínico' : 'Status Clínico'}: ${a.estado!.label(isEs ? 'es' : 'pt')}');
  }
  ...
  if (!hasP)
    buf.writeln(isEs
        ? '• (Sin conductas registradas)'
        : '• (Sem condutas registradas)');
  buf.writeln('');

  // ── Assinatura ──────────────────────────────────────────────────────────────
  final sigLine =
      nomeDoc.trim().isNotEmpty ? 'Dr/Dra. $nomeDoc' : '______________________';
  buf.writeln(
      '📅 ${isEs ? 'Fecha' : 'Data'}: ${_fmtFecha()} — ${_fmtHora()} hs');
  buf.writeln(
      '✍️  ${isEs ? 'Médico responsable' : 'Médico responsável'}: $sigLine');
  buf.writeln(sep);
...
String soapResumidoString(
  ...
  buf.writeln(sep);
  buf.writeln(
      '⚡ MEDCASES PRO - ${isEs ? 'RESUMEN EJECUTIVO' : 'RESUMO EXECUTIVO'}');
  buf.writeln(sep);
  ...
  buf.writeln(
      '👤 $nome • $idade ${isEs ? 'años' : 'anos'} • ${isEs ? 'Cama' : 'Leito'}: $cama • Dia $dia');
  ...
  if (s.alimentacion.isNotEmpty)
    syms.add('${isEs ? 'Alim' : 'Alim'}: ${s.alimentacion}');
  if (s.diuresis.isNotEmpty)
    syms.add('${isEs ? 'Diur' : 'Diur'}: ${s.diuresis}');
  if (s.evacuacion.isNotEmpty)
    syms.add('${isEs ? 'Evac' : 'Evac'}: ${s.evacuacion}');
  if (syms.isNotEmpty) {
  ...
  if (s.notePasaNoche.isNotEmpty) {
    final noteLines = s.notePasaNoche
        .split('\n')
        .map((l) => l.trim())
        .where((l) => l.isNotEmpty)
        .toList();
  ...
  if (ef.abdomen.isNotEmpty)
    efParts.add(isEs ? 'Abd: ${ef.abdomen}' : 'Abdome: ${ef.abdomen}');
  ...
  if (ex.imagenes.isNotEmpty) {
    buf.writeln(
        '• ${isEs ? 'Img' : 'Img'}: ${ex.imagenes.replaceAll('\n', ' ').trim()}');
  }
  if (ex.ecg.isNotEmpty) {
    buf.writeln('• ECG: ${ex.ecg}');
  }
  if (ex.culturas.isNotEmpty) {
    buf.writeln(
        '• ${isEs ? 'Cult' : 'Cult'}: ${ex.culturas.replaceAll('\n', ' ').trim()}');
  }
  if (o.tratamientoActual.isNotEmpty) {
    buf.writeln(
        '• ${isEs ? 'Trat. en curso' : 'Trat. em curso'}: ${o.tratamientoActual.replaceAll('\n', ' ').trim()}');
  }
  if (sv.isEmpty &&
      efParts.isEmpty &&
      ex.laboratorio.isEmpty &&
      ex.imagenes.isEmpty &&
      ex.ecg.isEmpty &&
      ex.culturas.isEmpty) {
    buf.writeln(isEs ? '• (Sin datos objetivos)' : '• (Sem dados objetivos)');
  ...
  if (a.estado != null) {
    buf.writeln(
        '• ${isEs ? 'Status' : 'Status'}: ${a.estado!.label(isEs ? 'es' : 'pt')}');
  }
  if (a.notasEvaluacion.isNotEmpty) {
  ...
  if (plan.planTerapeutico.isNotEmpty) {
    ...
    if (totalPlano > kMaxPlanoResumido) {
      buf.writeln(
          '• + ${totalPlano - kMaxPlanoResumido} outras condutas (ver prontuário)');
    }
  }
  if (plan.criteriosAlta.isNotEmpty) {
    hasP = true;
  ...
  if (ev.farmacos.isNotEmpty) {
    ...
    if (totalFarmacos > kMaxFarmacosResumido) {
      buf.writeln(
          '• + ${totalFarmacos - kMaxFarmacosResumido} outros fármacos (ver prescrição)');
    }
  }
  if (!hasP)
    buf.writeln(isEs
...
String soapPassagemString(
  ...
  buf.writeln(sep);
  buf.writeln(
      '🔄 MEDCASES PRO - ${isEs ? 'PASAJE DE GUARDIA' : 'PASSAGEM DE PLANTÃO'}');
  buf.writeln(sep);
  ...
  final dia = p?.diaInternacao ?? 1;
  buf.writeln(
      '🛏️ ${isEs ? 'LEITO' : 'LEITO'}: $cama • 👤 $nome (${idade}a/$sexo) • Dia $dia');
  ...
  if (o.tratamientoActual.isNotEmpty) {
    buf.writeln(
        '💊 ${isEs ? 'SUPORTE' : 'SUPORTE'}: ${o.tratamientoActual.replaceAll('\n', ' ').trim()}');
  } else {
    buf.writeln(
        '💊 ${isEs ? 'SUPORTE' : 'SUPORTE'}: ${isEs ? 'Sem suporte ativo registrado' : 'Sem suporte ativo registrado'}');
  }

  // ── LINHA 6 (opcional): Labs críticos comprimidos ──────────────────────────
  final labParts = <String>[];
  if (ex.laboratorio.isNotEmpty)
    labParts.add(ex.laboratorio.replaceAll('\n', ' ').trim());
  ...
  buf.writeln(
      '${isEs ? '🛑 PENDIENTES (TO-DO LIST):' : '🛑 PENDÊNCIAS (TO-DO LIST):'}');
  bool hasTodo = false;
  ...
  if (plan.planTerapeutico.isNotEmpty) {
    ...
    if (totalPlano > kMaxPlanoPassagem) {
      buf.writeln(
          '🚨 + ${totalPlano - kMaxPlanoPassagem} outras condutas (ver prontuário completo)');
    }
  }
  ...
  if (ev.farmacos.isNotEmpty) {
    ...
    if (totalFarmacos > kMaxFarmacosPassagem) {
      buf.writeln(
          '🚨 + ${totalFarmacos - kMaxFarmacosPassagem} outros fármacos (ver prescrição)');
    }
  }
