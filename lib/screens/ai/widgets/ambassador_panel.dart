...
class AmbassadorPanelState extends State<AmbassadorPanel> {
  ...
  void _copyLink() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          widget.lang == 'es' ? 'Link copiado 📋' : 'Link copiado 📋',
        ),
  ...
  Future<void> _runSecondOpinion() async {
    ...
    await widget.provider.sendAiMessage(
      fullPrompt,
      // M57A: VIP second opinion is a separate Study surface. It must not
      // silently inherit longResponse=false and become a Plantão bypass.
      longResponse: true,
      onChunk: (accumulated) {
