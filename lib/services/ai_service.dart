...
''';
  ...
  static String _buildPlantaoPhysicalRuntimeContract(String query, bool isEs) {
    ...
    if (isAcutePulmonaryEmbolism) {
      if (kDebugMode) {
        debugPrint(
          '[PLANTAO_PHYSICAL_RUNTIME_GUARD] '
          'type=acute_pulmonary_embolism lang=${isEs ? "es" : "pt"}',
        );
      }

      return isEs
          ? '[AUTORIDAD_FINAL_TEP_AGUDO]\n'
              'ENTIDAD EXPLICITA: tromboembolismo pulmonar agudo. Estratificar primero estabilidad hemodinamica; hipotension persistente/shock o paro atribuible a TEP define alto riesgo y cambia la prioridad hacia reperfusion urgente.\n'
              'Paciente estable: usar probabilidad clinica validada. Probabilidad baja/intermedia + D-dimero negativo ajustado cuando corresponda puede excluir TEP; probabilidad alta o D-dimero positivo requiere angio-TC pulmonar, salvo ruta alternativa por contraindicacion.\n'
              'TEP confirmado sin contraindicacion: iniciar anticoagulacion terapeutica; seleccionar heparina/LMWH/DOAC segun estabilidad, funcion renal, embarazo, cancer, riesgo de sangrado y posibilidad de reperfusion. No inventar farmaco/dosis sin contexto.\n'
              'TEP de alto riesgo con shock: reperfusion inmediata, habitualmente trombolisis sistemica si no hay contraindicacion. Si trombolisis esta contraindicada o falla, considerar embolectomia quirurgica o terapia dirigida por cateter en centro con experiencia.\n'
              'TEP intermedio-alto: anticoagulacion + monitorizacion estrecha por riesgo de deterioro; NO usar trombolisis sistemica primaria de rutina si permanece hemodinamicamente estable. Reperfusion de rescate si aparece descompensacion.\n'
              'Usar PESI/sPESI junto con funcion de VD y biomarcadores para estratificacion, no como sustituto de la evaluacion hemodinamica. Pacientes realmente de bajo riesgo pueden ser candidatos a alta/manejo ambulatorio con criterios estructurados.\n'
              'Si el paciente esta demasiado inestable para TC, ecocardiografia/ultrasonido a pie de cama puede apoyar decision urgente, pero ausencia de un hallazgo aislado no excluye TEP.\n\n'
          : '[AUTORIDADE_FINAL_TEP_AGUDO]\n'
              'ENTIDADE EXPLICITA: tromboembolismo pulmonar agudo. Estratificar primeiro estabilidade hemodinamica; hipotensao persistente/choque ou parada atribuivel a TEP define alto risco e muda a prioridade para reperfusao urgente.\n'
              'Paciente estavel: usar probabilidade clinica validada. Probabilidade baixa/intermediaria + D-dimero negativo ajustado quando indicado pode excluir TEP; probabilidade alta ou D-dimero positivo exige angio-TC pulmonar, salvo rota alternativa por contraindicacao.\n'
              'TEP confirmado sem contraindicacao: iniciar anticoagulacao terapeutica; selecionar heparina/HBPM/DOAC conforme estabilidade, funcao renal, gravidez, cancer, risco de sangramento e possibilidade de reperfusao. Nao inventar farmaco/dose sem contexto.\n'
              'TEP de alto risco com choque: reperfusao imediata, geralmente trombolise sistemica se nao houver contraindicacao. Se trombolise for contraindicada ou falhar, considerar embolectomia cirurgica ou terapia dirigida por cateter em centro experiente.\n'
              'TEP intermediario-alto: anticoagulacao + monitorizacao estreita pelo risco de deterioracao; NAO usar trombolise sistemica primaria rotineiramente se permanecer hemodinamicamente estavel. Reperfusao de resgate se houver descompensacao.\n'
              'Usar PESI/sPESI junto com funcao de VD e biomarcadores para estratificacao, nao como substituto da avaliacao hemodinamica. Pacientes realmente de baixo risco podem ser candidatos a alta/manejo ambulatorial com criterios estruturados.\n'
              'Se o paciente estiver instavel demais para TC, ecocardiografia/ultrassom a beira-leito pode apoiar decisao urgente, mas ausencia de achado isolado nao exclui TEP.\n\n';
    }
    ...
    final isPcr = folded.contains('parada cardiorrespiratoria') ||
        folded.contains('parada cardiaca') ||
        folded.contains('pcr');
    final isNonShockable = folded.contains('assistolia') ||
  ...
  static String buildClinicalSystemPrompt({
    ...
    if (isPlantaoMode) {
      ...
      final ptClinicalRegimenBlock = ptClinicalRegimenContract?.toPromptBlock(
    ...
    final String focusPt = switch (intentLabel) {
      ...
      'caso_clinico' => 'QUADRO ABERTO: 2-3 POSSIBILIDADES CLINICAS PRIORITARIAS. '
    ...
    final String focusEs = switch (intentLabel) {
      ...
      'caso_clinico' => 'CUADRO ABIERTO: 2-3 POSIBILIDADES CLINICAS PRIORITARIAS. '
