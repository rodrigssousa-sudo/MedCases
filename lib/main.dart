...
Future<void> main() async {
  ...
  // ── Compile-time identity diagnostic ───────────────────────────────────────
  // Emitted immediately after WidgetsFlutterBinding, before any async work.
  // These values are optional Docker build diagnostics and are never the
  // canonical production identity. On DigitalOcean Dockerfile builds, bindable
  // runtime variables are not available during the Flutter compilation stage.
  // The canonical identity is loaded from /deploy_meta.json after runApp().
  ...
  _loadDeployMeta(
    fallbackCompileSha: buildCommit,
    fallbackCompileBundleVersion: bundleVersion,
    fallbackCompileBuiltAt: builtAt,
  );
}
...
bool _isValidDeployCommit(String value) {
  return RegExp(r'^[0-9a-fA-F]{40,64}\$').hasMatch(value);
}

Future<void> _loadDeployMeta({
  required String fallbackCompileSha,
  required String fallbackCompileBundleVersion,
  required String fallbackCompileBuiltAt,
}) async {
  if (!kIsWeb) return;
  ...
  void logUnavailable(
    ...
    final statusSuffix = statusCode == null ? '' : ' httpStatus=$statusCode';
    ...
    debugPrint('[DEPLOY_IDENTITY][UNAVAILABLE] '
        ...
        'canonical=false '
        'fallbackCompileSha=$fallbackCompileSha '
        'fallbackCompileBundleVersion=$fallbackCompileBundleVersion '
        'fallbackCompileBuiltAt=$fallbackCompileBuiltAt');
  }
  ...
  try {
    ...
    if (decoded is! Map<String, dynamic>) {
      logUnavailable('invalid_payload');
      return;
    }

    final String status = decoded['status']?.toString() ?? 'unavailable';

    final String deployCommit =
        decoded['deployCommit']?.toString() ?? 'unknown';

    final String metaBundleVersion =
        decoded['bundleVersion']?.toString() ?? 'unknown';

    final String containerStartedAt =
        decoded['containerStartedAt']?.toString() ?? 'unknown';

    if (status != 'available') {
      logUnavailable('metadata_status_\$status');
      return;
    }

    if (!_isValidDeployCommit(deployCommit)) {
      logUnavailable('invalid_deploy_commit');
      return;
    }

    final bool compileShaComparable = _isValidDeployCommit(fallbackCompileSha);

    final String compileMatch = compileShaComparable
        ? (fallbackCompileSha.toLowerCase() == deployCommit.toLowerCase())
            .toString()
        : 'not_comparable';
    ...
    logUnavailable('fetch_or_parse_failure');
  }
}
...
class _AuthGateState extends State<_AuthGate> {
  ...
  void _signalSplashReady(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      final splashState = context.findAncestorStateOfType<_TimedSplashState>();
      splashState?._signalAuthResolved();
    });
  }
