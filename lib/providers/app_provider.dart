...
import '../services/ai_pipeline/plantao/contracts/plantao_request.dart';
import '../services/ai_pipeline/plantao/contracts/plantao_section.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_request_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_finalization_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_validation_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_persistence_shadow_adapter.dart';
...
import '../services/ai_pipeline/plantao/shadow/plantao_drug_evidence_request_observer.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_evidence_finalization_join_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_identity_provenance_binding_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_provenance_persistence_payload_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_provenance_persistence_payload_parity_gate_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_provenance_semantic_equality_gate_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_provenance_persistence_review_envelope_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_persistence_cutover_blocker_audit_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_typed_regimen_capability_matrix_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_explicit_typed_regimen_evidence_projection_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_projected_evidence_bundle_overlay_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/shadow/plantao_drug_projected_evidence_validation_replay_comparison_shadow_adapter.dart';
import '../services/ai_pipeline/plantao/plantao_buffered_cutover_controller.dart';
...
import '../services/ai_pipeline/plantao/plantao_qa_cutover_support.dart';
...
class AppProvider extends ChangeNotifier {
  ...
  bool _plantaoBufferedCutoverExecutionActive = false;
  String? _plantaoBufferedPipelineCancellationRequestId;
  void Function()? _plantaoBufferedPipelineCancellation;
  final PlantaoQaCutoverSupport _plantaoQaCutoverSupport =
      const PlantaoQaCutoverSupport();

  @visibleForTesting
  void configurePlantaoBufferedCutoverForTesting(
    PlantaoBufferedCutoverController controller,
  ) {
    _plantaoBufferedCutoverController = controller;
  }

  static String resolveEffectiveAiPriorityForRouting({
    required bool isPlantaoMode,
    required bool geminiConnected,
    required String aiPriority,
    ...
    return geminiConnected ? 'academic' : aiPriority;
  }
  ...
  void setActiveImportedPatient(PacienteSession session) {
    final patientKey = session.sessionKey.trim();
    final patientName = session.paciente.nome.trim();

    if (patientKey.isEmpty || patientName.isEmpty) {
    ...
    activeImportedPatientKey = patientKey;
    // Ponteiro interno: não notifica ouvintes nem cria uma segunda fonte de estado.
  }

  bool get hasWritableImportedPatient {
    final session = activeImportedSession;
    final activeKey = activeImportedPatientKey?.trim() ?? '';
    if (session == null || activeKey.isEmpty) return false;

    final sessionKey = session.sessionKey.trim();
    final patientName = session.paciente.nome.trim();
    return patientName.isNotEmpty && sessionKey == activeKey;
  }

  String? get writableImportedPatientKey =>
      hasWritableImportedPatient ? activeImportedPatientKey!.trim() : null;
  ...
  Future<void> _setUserImpl(UserModel user) async {
    ...
    try {
      ...
      if (!FirebaseRuntimeGuard.isReady) {
        ...
        debugPrint(
          '[AUTH_CONVERGENCE][FAILED] reason=firebase_unavailable '
      ...
      if (fbSdkUser == null && restTokenPresent) {
        debugPrint(
          '[AUTH_CONVERGENCE][WAITING_FOR_SDK] '
      ...
      if (fbSdkUser == null) {
        ...
        debugPrint(
          '[AUTH_CONVERGENCE][AUTH_REQUIRED] '
        ...
        debugPrint(
          '[AUTH_CONVERGENCE][FAILED] reason=uid_mismatch '
          'expectedUid=$expectedUid firebaseUid=${fbSdkUser.uid}',
        );
      ...
      debugPrint(
        '[AUTH_CONVERGENCE][FAILED] reason=auth_error '
        ...
        try {
          ...
          if (decoded is List) {
            _myHistories = decoded
                .whereType<Map>()
                .map(
                  (e) => ClinicalHistoryModel.fromJson(
                    e is Map<String, dynamic>
                        ? e
                        : Map<String, dynamic>.from(e),
                  ),
  ...
  void updateDkahhsHighRiskStatus(DkahhsHighRiskStatus v) {
    _dkahhsHighRiskStatus = v;
    notifyListeners();
  }

  void updateDkahhsPotassiumObservation(DkahhsPotassiumObservation? v) {
    _dkahhsPotassiumObservation = v;
    notifyListeners();
  }

  void recordDkahhsGlucoseSample(DkahhsGlucoseSample v) {
    _dkahhsPreviousGlucoseSample = _dkahhsCurrentGlucoseSample;
    _dkahhsCurrentGlucoseSample = v;
    notifyListeners();
  }

  void clearDkahhsSafetyContext() {
    _resetDkahhsSafetyContext();
    notifyListeners();
  }

  void _resetDkahhsSafetyContext() {
    _dkahhsPregnancyStatus = DkahhsPregnancyStatus.unknown;
    _dkahhsHighRiskStatus = DkahhsHighRiskStatus.unknown;
    _dkahhsPotassiumObservation = null;
    _dkahhsPreviousGlucoseSample = null;
    _dkahhsCurrentGlucoseSample = null;
  }
  ...
  void _onFavWriteFailure(FirestoreWriteResult result, String operation) {
    debugPrint(
      '[WRITE_BARRIER_REVERT] operation=$operation '
    ...
    debugPrint(
      '[OPERATIONAL_ERROR] write_back_frozen=true '
  ...
  void removePlantaoPatient(String id) {
    ...
    final uid = _currentUser?.uid;
    if (uid != null) {
      InternacionFirestoreService.softDelete(
        uid,
        sessionKey,
      ).catchError((_) {});
    }
  ...
  Future<void> loadHistories() async {
    ...
    _historiesStreamSub = FirestoreService.streamHistories(uid).listen(
      ...
      onError: (_) {
        /* Stream error: dados em memória preservados */
      },
    );
  }
      ...
      FirestoreService.saveHistoryTyped(uid, h).then((payload) {
        ...
        if (uploadedAt != null && uploadedAt.isNotEmpty && h.isPublic) {
          ...
          if (pubIdx >= 0 &&
            ...
            _publicHistories[pubIdx] = _publicHistories[pubIdx].copyWith(
              uploadedAt: uploadedAt,
            );
            final myIdx = _myHistories.indexWhere((x) => x.id == h.id);
            if (myIdx >= 0) {
              _myHistories[myIdx] = _myHistories[myIdx].copyWith(
                uploadedAt: uploadedAt,
              );
            }
            _saveHistoriesLocal(uid).catchError((_) {});
    ...
    if (syncFuture != null) {
      try {
        await syncFuture.timeout(
          ...
          onTimeout: () {
            debugPrint(
              '[BUILD293][rebuildAiHistory] sync timeout 6s — '
  ...
  Future<bool?> connectGemini() async {
    ...
    try {
      ...
      final ok = await GeminiService.signIn().timeout(
  ...
  void _setGeminiConnectionState({
    ...
    if (notify && connected && !stateChanged) {
      debugPrint(
        '[BUILD336] _setGeminiConnectionState: estado inalterado mas '
  ...
  Future<bool> _ensureGeminiApiKey({required String source}) async {
    ...
    try {
      final geminiKey = await FirestoreService.loadGeminiApiKey().timeout(
  ...
  Future<void> checkGeminiSession() {
    ...
    final future = () async {
      try {
        ...
        if (!isPrivileged && !hasPendingOAuthRedirect) {
          ...
          if (!_apiKeyWipedThisSession) {
            ...
            try {
              ...
              debugPrint(
                '[BUILD294][SecurityWipe] wiped=$wiped '
        ...
        final connected = await GeminiService.isConnected().timeout(
          const Duration(seconds: 5),
          onTimeout: () => false,
        );
        if (connected) {
  ...
  // PHASE 3C — request shadow only. Never rendered, persisted or executed.
  PlantaoRequest? _lastPlantaoShadowRequest;

  @visibleForTesting
  PlantaoRequest? get lastPlantaoShadowRequest => _lastPlantaoShadowRequest;

  // PHASE 3D — canonical finalization and structure, observational only.
  final PlantaoFinalizationShadowAdapter _plantaoFinalizationShadowAdapter =
      PlantaoFinalizationShadowAdapter.withExistingImplementations();
  PlantaoFinalizationShadowSnapshot? _lastPlantaoFinalizationShadow;

  @visibleForTesting
  PlantaoFinalizationShadowSnapshot? get lastPlantaoFinalizationShadow =>
      _lastPlantaoFinalizationShadow;

  // PHASE 3E — provider route plan and deterministic validation, shadow only.
  final PlantaoValidationShadowAdapter _plantaoValidationShadowAdapter =
      const PlantaoValidationShadowAdapter();
  PlantaoValidationShadowSnapshot? _lastPlantaoValidationShadow;

  @visibleForTesting
  PlantaoValidationShadowSnapshot? get lastPlantaoValidationShadow =>
      _lastPlantaoValidationShadow;

  // PHASE 3G — bind the legacy protocol retrieval to typed evidence.
  final PlantaoRetrievalShadowAdapter _plantaoRetrievalShadowAdapter =
      const PlantaoRetrievalShadowAdapter();
  PlantaoRetrievalShadowSnapshot? _lastPlantaoRetrievalShadow;

  @visibleForTesting
  PlantaoRetrievalShadowSnapshot? get lastPlantaoRetrievalShadow =>
      _lastPlantaoRetrievalShadow;

  // PHASE 3H — canonical drug evidence from original input, shadow only.
  final PlantaoRemoteDrugEvidenceRuntimeObserver
      _plantaoDrugEvidenceRuntimeObserver =
      PlantaoRemoteDrugEvidenceRuntimeObserver();
  final PlantaoShadowExecutionIsolator _plantaoShadowExecutionIsolator =
      const PlantaoShadowExecutionIsolator();
  late final PlantaoVersionedRemoteDrugEvidenceJsonLoader
  ...
  void refreshPlantaoDrugEvidenceShadowAfterLifecycleChange() {
    _plantaoShadowExecutionIsolator.launch(
      () => _plantaoDrugEvidenceRemoteLoader.refreshCurrentPointer(),
      onFailure: (error, stackTrace) {
        debugPrint(
          '[PLANTAO_SHADOW_POINTER_REFRESH][FAILED] '
  ...
  late final PlantaoGeneratedDrugEvidenceReadOnlyAdapter
      _plantaoDrugEvidencePort = PlantaoGeneratedDrugEvidenceReadOnlyAdapter(
    loadJsonText: _plantaoDrugEvidenceRemoteLoader.loadJsonText,
  );
  late final PlantaoDrugEvidenceRequestObserver
      _plantaoDrugEvidenceRequestObserver = PlantaoDrugEvidenceRequestObserver(
    adapter: PlantaoDrugEvidenceShadowAdapter(port: _plantaoDrugEvidencePort),
  );
  PlantaoDrugEvidenceShadowSnapshot? _lastPlantaoDrugEvidenceShadow;
  Future<PlantaoDrugEvidenceRequestSnapshot>? _plantaoDrugEvidenceRequestFuture;
  PlantaoDrugEvidenceRequestSnapshot? _lastPlantaoDrugEvidenceRequestShadow;
  final PlantaoDrugEvidenceFinalizationJoinShadowAdapter
      _plantaoDrugEvidenceFinalizationJoinShadowAdapter =
      const PlantaoDrugEvidenceFinalizationJoinShadowAdapter();
  PlantaoDrugEvidenceFinalizationJoinShadowSnapshot?
      _lastPlantaoDrugEvidenceFinalizationJoinShadow;
  final PlantaoDrugIdentityProvenanceBindingShadowAdapter
      _plantaoDrugIdentityProvenanceBindingShadowAdapter =
      const PlantaoDrugIdentityProvenanceBindingShadowAdapter();
  PlantaoDrugIdentityProvenanceBindingShadowSnapshot?
      _lastPlantaoDrugIdentityProvenanceBindingShadow;
  final PlantaoDrugProvenancePersistencePayloadShadowAdapter
      _plantaoDrugProvenancePersistencePayloadShadowAdapter =
      const PlantaoDrugProvenancePersistencePayloadShadowAdapter();
  PlantaoDrugProvenancePersistencePayloadShadowSnapshot?
      _lastPlantaoDrugProvenancePersistencePayloadShadow;
  final PlantaoDrugProvenancePersistencePayloadParityGateShadowAdapter
      _plantaoDrugProvenancePersistencePayloadParityGateShadowAdapter =
  ...
  PlantaoDrugProvenancePersistencePayloadParityShadowSnapshot?
      _lastPlantaoDrugProvenancePersistencePayloadParityShadow;
  final PlantaoDrugProvenanceSemanticEqualityGateShadowAdapter
      _plantaoDrugProvenanceSemanticEqualityGateShadowAdapter =
      const PlantaoDrugProvenanceSemanticEqualityGateShadowAdapter();
  PlantaoDrugProvenanceSemanticEqualityShadowSnapshot?
      _lastPlantaoDrugProvenanceSemanticEqualityShadow;
  final PlantaoDrugProvenancePersistenceReviewEnvelopeShadowAdapter
      _plantaoDrugProvenancePersistenceReviewEnvelopeShadowAdapter =
  ...
  PlantaoDrugProvenancePersistenceReviewEnvelopeShadowSnapshot?
      _lastPlantaoDrugProvenancePersistenceReviewEnvelopeShadow;
  final PlantaoDrugPersistenceCutoverBlockerAuditShadowAdapter
      _plantaoDrugPersistenceCutoverBlockerAuditShadowAdapter =
      const PlantaoDrugPersistenceCutoverBlockerAuditShadowAdapter();
  PlantaoDrugPersistenceCutoverBlockerAuditShadowSnapshot?
      _lastPlantaoDrugPersistenceCutoverBlockerAuditShadow;
  final PlantaoDrugTypedRegimenCapabilityMatrixShadowAdapter
      _plantaoDrugTypedRegimenCapabilityMatrixShadowAdapter =
      const PlantaoDrugTypedRegimenCapabilityMatrixShadowAdapter();
  PlantaoDrugTypedRegimenCapabilityMatrixShadowSnapshot?
      _lastPlantaoDrugTypedRegimenCapabilityMatrixShadow;
  final PlantaoDrugExplicitTypedRegimenEvidenceProjectionShadowAdapter
      _plantaoDrugExplicitTypedRegimenEvidenceProjectionShadowAdapter =
  ...
  PlantaoDrugExplicitTypedRegimenEvidenceProjectionShadowSnapshot?
      _lastPlantaoDrugExplicitTypedRegimenEvidenceProjectionShadow;
  final PlantaoDrugProjectedEvidenceBundleOverlayShadowAdapter
      _plantaoDrugProjectedEvidenceBundleOverlayShadowAdapter =
  ...
  final PlantaoDrugProjectedEvidenceValidationReplayComparisonShadowAdapter
  ...
  PlantaoDrugProjectedEvidenceValidationReplayComparisonShadowSnapshot?
      _lastPlantaoDrugProjectedEvidenceValidationReplayComparisonShadow;

  @visibleForTesting
  PlantaoDrugEvidenceShadowSnapshot? get lastPlantaoDrugEvidenceShadow =>
      _lastPlantaoDrugEvidenceShadow;

  @visibleForTesting
  PlantaoDrugEvidenceRequestSnapshot?
      get lastPlantaoDrugEvidenceRequestShadow =>
          _lastPlantaoDrugEvidenceRequestShadow;

  @visibleForTesting
  PlantaoDrugEvidenceFinalizationJoinShadowSnapshot?
      get lastPlantaoDrugEvidenceFinalizationJoinShadow =>
          _lastPlantaoDrugEvidenceFinalizationJoinShadow;

  @visibleForTesting
  PlantaoDrugIdentityProvenanceBindingShadowSnapshot?
      get lastPlantaoDrugIdentityProvenanceBindingShadow =>
          _lastPlantaoDrugIdentityProvenanceBindingShadow;

  @visibleForTesting
  PlantaoDrugProvenancePersistencePayloadShadowSnapshot?
      get lastPlantaoDrugProvenancePersistencePayloadShadow =>
          _lastPlantaoDrugProvenancePersistencePayloadShadow;

  @visibleForTesting
  PlantaoDrugProvenancePersistencePayloadParityShadowSnapshot?
      get lastPlantaoDrugProvenancePersistencePayloadParityShadow =>
          _lastPlantaoDrugProvenancePersistencePayloadParityShadow;

  @visibleForTesting
  PlantaoDrugProvenanceSemanticEqualityShadowSnapshot?
      get lastPlantaoDrugProvenanceSemanticEqualityShadow =>
          _lastPlantaoDrugProvenanceSemanticEqualityShadow;

  @visibleForTesting
  PlantaoDrugProvenancePersistenceReviewEnvelopeShadowSnapshot?
      get lastPlantaoDrugProvenancePersistenceReviewEnvelopeShadow =>
          _lastPlantaoDrugProvenancePersistenceReviewEnvelopeShadow;

  @visibleForTesting
  PlantaoDrugPersistenceCutoverBlockerAuditShadowSnapshot?
      get lastPlantaoDrugPersistenceCutoverBlockerAuditShadow =>
          _lastPlantaoDrugPersistenceCutoverBlockerAuditShadow;

  @visibleForTesting
  PlantaoDrugTypedRegimenCapabilityMatrixShadowSnapshot?
      get lastPlantaoDrugTypedRegimenCapabilityMatrixShadow =>
          _lastPlantaoDrugTypedRegimenCapabilityMatrixShadow;

  @visibleForTesting
  PlantaoDrugExplicitTypedRegimenEvidenceProjectionShadowSnapshot?
      get lastPlantaoDrugExplicitTypedRegimenEvidenceProjectionShadow =>
          _lastPlantaoDrugExplicitTypedRegimenEvidenceProjectionShadow;

  @visibleForTesting
  PlantaoDrugProjectedEvidenceBundleOverlayShadowSnapshot?
      get lastPlantaoDrugProjectedEvidenceBundleOverlayShadow =>
          _lastPlantaoDrugProjectedEvidenceBundleOverlayShadow;

  @visibleForTesting
  PlantaoDrugProjectedEvidenceValidationReplayComparisonShadowSnapshot?
      get lastPlantaoDrugProjectedEvidenceValidationReplayComparisonShadow =>
          _lastPlantaoDrugProjectedEvidenceValidationReplayComparisonShadow;

  // PHASE 3F — persistence-ready record and provenance, shadow only.
  final PlantaoPersistenceShadowAdapter _plantaoPersistenceShadowAdapter =
      const PlantaoPersistenceShadowAdapter();
  PlantaoPersistenceShadowSnapshot? _lastPlantaoPersistenceShadow;

  @visibleForTesting
  PlantaoPersistenceShadowSnapshot? get lastPlantaoPersistenceShadow =>
      _lastPlantaoPersistenceShadow;

  void _capturePlantaoRetrievalShadow({
    required PlantaoRequest request,
    required Iterable<ProtocolModel> matchedProtocols,
  }) {
    try {
      final matches = matchedProtocols.map(
        (protocol) => PlantaoLegacyProtocolMatch(
          documentId: 'protocol:${protocol.id}',
          version: 'legacy_protocols_database_v1',
          title: tDB(protocol.title),
          recognize: tDB(protocol.recognize),
          definition: tDB(protocol.definition),
          actions: protocol.getActions(_lang),
      ...
      _lastPlantaoRetrievalShadow = _plantaoRetrievalShadowAdapter
          .bindLegacyProtocolMatches(request: request, matches: matches);
    } catch (error, stackTrace) {
      _lastPlantaoRetrievalShadow = null;
      debugPrint(
        '[PLANTAO_SHADOW_RETRIEVAL][INVALID] requestId=${request.requestId} '
        'error=$error stack=$stackTrace',
      );
    }
  }

  void _capturePlantaoDrugEvidenceShadow({
    required PlantaoRequest request,
    required String originalUserInput,
  }) {
    final legacyQueryIntent = _classifyIntent(originalUserInput);
    final legacyDirectQuery = _isDirectQuery(originalUserInput);

    final requestFuture = _plantaoDrugEvidenceRequestObserver.observeRequest(
    ...
    _plantaoDrugEvidenceRequestFuture = requestFuture;

    _plantaoShadowExecutionIsolator.launch(
      () async {
        final requestSnapshot = await requestFuture;

        // Late network observations can never overwrite a newer request.
        if (_lastPlantaoShadowRequest?.requestId != request.requestId) {
          return;
        }
        _lastPlantaoDrugEvidenceRequestShadow = requestSnapshot;
        _lastPlantaoDrugEvidenceShadow = requestSnapshot.evidence;
        assert(() {
          final snapshot = requestSnapshot.evidence;
          debugPrint(
            '[PLANTAO_SHADOW_DRUG_EVIDENCE] requestId=${request.requestId} '
            'status=${snapshot.status.name} '
            'candidates=${snapshot.candidates.length} '
            'documents=${snapshot.documents.length} '
            'materializable=${snapshot.documents.where((item) => item.supportsMedicationMaterialization).length}',
          );
          return true;
        }());
      },
      onFailure: (error, stackTrace) {
        ...
        _lastPlantaoDrugEvidenceRequestShadow = null;
        _lastPlantaoDrugEvidenceShadow = null;
        debugPrint(
          '[PLANTAO_SHADOW_DRUG_EVIDENCE][FAILED] '
          'requestId=${request.requestId} error=$error stack=$stackTrace',
        );
      },
    );
  }
  ...
  Future<void> _capturePlantaoFinalizationShadow({
    ...
    required ClinicalStructuredOutput? clinicalOutput,
  }) async {
    final snapshot = await _plantaoFinalizationShadowAdapter.finalize(
      request: request,
      finalText: finalText,
      structuredOutput: clinicalOutput,
      providerLabel: 'observed_productive_output',
    );

    // Late shadow work can never overwrite a newer productive request.
    if (_lastPlantaoShadowRequest?.requestId != request.requestId) {
      return;
    }

    _lastPlantaoFinalizationShadow = snapshot;
    final retrievalSnapshot =
        _lastPlantaoRetrievalShadow?.requestId == request.requestId
            ? _lastPlantaoRetrievalShadow
            : null;
    final validationSnapshot = retrievalSnapshot == null
        ? _plantaoValidationShadowAdapter.observeWithoutRetrieval(
            request: request,
        ...
        : _plantaoValidationShadowAdapter.validate(
            request: request,
            finalization: snapshot,
            evidenceBundle: retrievalSnapshot.evidenceBundle,
          );

    // Validation is observational. It cannot type medications without a typed
    // candidate and deterministic evidence, and never changes productive text.
    _lastPlantaoValidationShadow = validationSnapshot;
    final persistenceSnapshot = _plantaoPersistenceShadowAdapter.observe(
      request: request,
      finalization: snapshot,
    ...
    // A newer productive request invalidates the late observational join.
    if (_lastPlantaoShadowRequest?.requestId != request.requestId) {
      return;
    }
    ...
    final drugIdentityProvenanceBinding =
      ...
      request: request,
      validationRequestId: validationSnapshot.requestId,
      validationProvenance: validationSnapshot.provenance,
      join: drugEvidenceJoin,
    );
    _lastPlantaoDrugIdentityProvenanceBindingShadow =
        drugIdentityProvenanceBinding;
    final drugProvenancePersistencePayload =
        _plantaoDrugProvenancePersistencePayloadShadowAdapter.prepare(
      request: request,
      basePersistencePayload: persistenceSnapshot.record.toJson(),
      baseFuturePersistenceEligible:
          persistenceSnapshot.futurePersistenceEligible,
      binding: drugIdentityProvenanceBinding,
    );
    _lastPlantaoDrugProvenancePersistencePayloadShadow =
        drugProvenancePersistencePayload;
    final drugProvenancePersistencePayloadParity =
        _plantaoDrugProvenancePersistencePayloadParityGateShadowAdapter.verify(
      request: request,
      basePersistencePayload: persistenceSnapshot.record.toJson(),
      baseFuturePersistenceEligible:
          persistenceSnapshot.futurePersistenceEligible,
      preparedPayload: drugProvenancePersistencePayload,
      binding: drugIdentityProvenanceBinding,
    );
    _lastPlantaoDrugProvenancePersistencePayloadParityShadow =
        drugProvenancePersistencePayloadParity;
    final drugProvenanceSemanticEquality =
        _plantaoDrugProvenanceSemanticEqualityGateShadowAdapter.verify(
      request: request,
      baseFuturePersistenceEligible:
          persistenceSnapshot.futurePersistenceEligible,
      preparedPayload: drugProvenancePersistencePayload,
      parity: drugProvenancePersistencePayloadParity,
      binding: drugIdentityProvenanceBinding,
    );
    _lastPlantaoDrugProvenanceSemanticEqualityShadow =
        drugProvenanceSemanticEquality;
    final drugProvenancePersistenceReviewEnvelope =
      ...
      preparedPayload: drugProvenancePersistencePayload,
      parity: drugProvenancePersistencePayloadParity,
      semanticEquality: drugProvenanceSemanticEquality,
    );
    _lastPlantaoDrugProvenancePersistenceReviewEnvelopeShadow =
        drugProvenancePersistenceReviewEnvelope;
    final drugPersistenceCutoverBlockerAudit =
        _plantaoDrugPersistenceCutoverBlockerAuditShadowAdapter.audit(
      request: request,
      baseFuturePersistenceEligible:
          persistenceSnapshot.futurePersistenceEligible,
      reviewEnvelope: drugProvenancePersistenceReviewEnvelope,
      binding: drugIdentityProvenanceBinding,
    );
    _lastPlantaoDrugPersistenceCutoverBlockerAuditShadow =
        drugPersistenceCutoverBlockerAudit;
    final drugTypedRegimenCapabilityMatrix =
        _plantaoDrugTypedRegimenCapabilityMatrixShadowAdapter.audit(
      request: request,
      join: drugEvidenceJoin,
      binding: drugIdentityProvenanceBinding,
      cutoverBlockerAudit: drugPersistenceCutoverBlockerAudit,
    );
    _lastPlantaoDrugTypedRegimenCapabilityMatrixShadow =
        drugTypedRegimenCapabilityMatrix;
    final drugExplicitTypedRegimenEvidenceProjection =
        _plantaoDrugExplicitTypedRegimenEvidenceProjectionShadowAdapter.project(
      request: request,
      join: drugEvidenceJoin,
      capabilityMatrix: drugTypedRegimenCapabilityMatrix,
    );
    _lastPlantaoDrugExplicitTypedRegimenEvidenceProjectionShadow =
    ...
    final projectedEvidenceReplayValidation = drugProjectedEvidenceBundleOverlay
        ...
        ? _plantaoValidationShadowAdapter.validate(
            request: request,
            finalization: snapshot,
            evidenceBundle: drugProjectedEvidenceBundleOverlay.candidateBundle!,
          )
        : null;
    final drugProjectedEvidenceValidationReplayComparison =
        _plantaoDrugProjectedEvidenceValidationReplayComparisonShadowAdapter
            .compare(
      request: request,
      overlayPrepared: drugProjectedEvidenceBundleOverlay.overlayPrepared,
      originalValidationRequestId: validationSnapshot.requestId,
      originalValidationStatus: validationSnapshot.status.name,
      originalProvenance: validationSnapshot.provenance.toJson(),
      originalRouteProviders: validationSnapshot.routePlan.attempts.map(
        (item) => item.provider.name,
      ),
      replayValidationRequestId: projectedEvidenceReplayValidation?.requestId,
      replayValidationStatus: projectedEvidenceReplayValidation?.status.name,
      replayProvenance: projectedEvidenceReplayValidation?.provenance.toJson(),
      replayRouteProviders: projectedEvidenceReplayValidation
          ?.routePlan.attempts
          .map((item) => item.provider.name),
    );
    _lastPlantaoDrugProjectedEvidenceValidationReplayComparisonShadow =
        drugProjectedEvidenceValidationReplayComparison;

    assert(() {
      debugPrint(
        '[PLANTAO_SHADOW_FINALIZATION] requestId=${snapshot.requestId} '
        'status=${snapshot.status.name} '
        'canonicalStatus=${snapshot.canonicalStatus} '
        'sections=${snapshot.structure?.sections.length ?? 0} '
        'deferredMedications=${snapshot.deferredMedicationCount} '
        'retrieval=${retrievalSnapshot?.evidenceBundle.retrievalStatus.name ?? "unavailable"} '
        'protocolDocs=${retrievalSnapshot?.evidenceBundle.protocolDocuments.length ?? 0} '
        'drugJoin=${drugEvidenceJoin.status.name} '
        'drugDocs=${drugEvidenceJoin.evidenceDocumentIds.length} '
        'drugProvenance=${drugIdentityProvenanceBinding.status.name} '
        'drugTypedRegimens=${drugIdentityProvenanceBinding.typedRegimenDocumentIds.length} '
        'drugPersistencePayload=${drugProvenancePersistencePayload.status.name} '
        'drugPersistenceEligible=${drugProvenancePersistencePayload.baseFuturePersistenceEligible} '
        'drugPersistenceParity=${drugProvenancePersistencePayloadParity.status.name} '
        'drugPersistenceUnexpected=${drugProvenancePersistencePayloadParity.unexpectedPaths.length} '
        'drugProvenanceSemantic=${drugProvenanceSemanticEquality.status.name} '
        'drugProvenanceSemanticMismatches=${drugProvenanceSemanticEquality.mismatchPaths.length} '
        'drugPersistenceReview=${drugProvenancePersistenceReviewEnvelope.status.name} '
        'drugPersistenceReviewEligible=${drugProvenancePersistenceReviewEnvelope.baseFuturePersistenceEligible} '
        'drugCutoverAudit=${drugPersistenceCutoverBlockerAudit.status.name} '
        'drugCutoverBlockers=${drugPersistenceCutoverBlockerAudit.blockers.length} '
        'drugTypedRegimenAvailable=${drugPersistenceCutoverBlockerAudit.hasTypedRegimenEvidence} '
        'drugRegimenMatrix=${drugTypedRegimenCapabilityMatrix.status.name} '
        'drugRegimenDocuments=${drugTypedRegimenCapabilityMatrix.canonicalDocumentCount} '
        'drugRegimenCapable=${drugTypedRegimenCapabilityMatrix.typedRegimenCapableCount} '
        'drugRegimenGaps=${drugTypedRegimenCapabilityMatrix.unresolvedGaps.length} '
        'drugRegimenProjection=${drugExplicitTypedRegimenEvidenceProjection.status.name} '
        'drugRegimensProjected=${drugExplicitTypedRegimenEvidenceProjection.projectedRegimenCount} '
        'drugRegimenUnavailable=${drugExplicitTypedRegimenEvidenceProjection.unavailableDocumentIds.length} '
        'drugRegimenRejected=${drugExplicitTypedRegimenEvidenceProjection.rejectedEntries.length} '
        'drugEvidenceOverlay=${drugProjectedEvidenceBundleOverlay.status.name} '
        'drugEvidenceOverlayDocs=${drugProjectedEvidenceBundleOverlay.candidateDrugDocumentCount} '
        'drugEvidenceOverlayAdded=${drugProjectedEvidenceBundleOverlay.addedRegimenKeys.length} '
        'drugEvidenceOverlayDuplicates=${drugProjectedEvidenceBundleOverlay.duplicateRegimenKeys.length} '
        'drugValidationReplay=${drugProjectedEvidenceValidationReplayComparison.status.name} '
        'drugValidationReplayStatusChanged=${drugProjectedEvidenceValidationReplayComparison.statusChanged} '
        'drugValidationReplayProvenanceChanged=${drugProjectedEvidenceValidationReplayComparison.provenanceChanged} '
        'drugValidationReplayRouteChanged=${drugProjectedEvidenceValidationReplayComparison.routeChanged} '
        'validation=${validationSnapshot.status.name} '
        'persistence=${persistenceSnapshot.record.status.name} '
        'futurePersistenceEligible=${persistenceSnapshot.futurePersistenceEligible} '
        'route=${validationSnapshot.routePlan.attempts.map((item) => item.provider.name).join(">")}',
      );
      return true;
    }());
  }
  ...
  static const Set<String> qaTesterUids = {'Wa1AQN8hvCdewLiR2drd01rQo9G3'};
  ...
  bool get shouldForceGptFallbackForQa {
    ...
    switch (result) {
      ...
      case _QaGateResult.authorizedTester:
        // ignore: avoid_print
        print(
          '[AI_QA_GATE] enabled=true authorized=true reason=tester_uid '
  ...
  void cancelAiStream({bool notifyBufferedPipeline = true}) {
    final cancelledRequestId = _activeRequestId.trim();
    final bufferedCancellation = _plantaoBufferedPipelineCancellation;
    _plantaoBufferedPipelineCancellationRequestId = null;
    _plantaoBufferedPipelineCancellation = null;
    ...
    if (wasGptActive) {
      // ignore: avoid_print
      print(
        '[AI_E2E][CANCELLED] requestId=$cancelledRequestId '
    ...
    if (cancelledRequestId.isNotEmpty &&
        _activeRequestId == cancelledRequestId) {
      _activeRequestId = '';
      _completeAiRequestOnce(cancelledRequestId);
    }
    ...
    // PHASE3K-C5A-R8: o cancelamento externo encerra também o stream tipado.
    // O callback roda em microtask para permitir que a UI invalide sua geração
    // imediatamente após cancelAiStream() retornar.
    if (notifyBufferedPipeline && bufferedCancellation != null) {
      scheduleMicrotask(bufferedCancellation);
    }
  }
  ...
  /// AI-RECONSTRUCTION-R18.6AB-R1B-R3-R1:
  /// Adota como ativa uma conversa selecionada no histórico.
  ///
  ...
  void adoptRestoredAiConversation({
    ...
    if (normalizedSessionId.isEmpty) {
      throw ArgumentError.value(
        sessionId,
        'sessionId',
        'A sessão restaurada deve possuir um ID não vazio.',
      );
    }
    ...
    if (normalizedTitle.isNotEmpty) {
      _currentConversationTitle = normalizedTitle;
    }

    // A conversa restaurada já possui ao menos um turno persistido.
    _isFirstMessageOfSession = false;

    debugPrint(
      '[SESSION_RESTORE][PROVIDER_ADOPTED] '
      'sessionIdHash=${normalizedSessionId.hashCode} '
      'titleLen=${_currentConversationTitle.length}',
    );
  }
  ...
  final Set<String> _deletedAiSessionIds = <String>{};

  /// MICRO-BUILD 462E-A.5.3.7.3.2.5.2 [PILLAR 2]:
  /// Single reactive selector — deduplicated, sorted by updatedAt desc, ≤20.
  ...
  // AI-RECONSTRUCTION-R18.6X-R1-R1:
  // Remove do read model uma sessão cuja exclusão já foi confirmada.
  // O marcador local impede reentrada por hidratação iniciada anteriormente.
  bool removeVisibleAiSessionSummary(String sessionId) {
    _deletedAiSessionIds.add(sessionId);

    final typedBefore = _localAiSessionSummaries.length;
    final rawBefore = _localAiSessionIndex.length;

    _localAiSessionSummaries.removeWhere(
      (summary) => summary.sessionId == sessionId,
    );

    _localAiSessionIndex.removeWhere(
      (summary) => summary['sessionId'] == sessionId,
    );

    final changed = typedBefore != _localAiSessionSummaries.length ||
        rawBefore != _localAiSessionIndex.length;
  ...
  void _completeAiRequestOnce(String requestId) {
    ...
    if (_completedRequestIds.contains(requestId)) {
      // ignore: avoid_print
      debugPrint(
        '[RESUME_COORDINATOR][DUPLICATE_DROPPED] '
        'requestId=$requestId '
    ...
    if (!_completedResolutions.containsKey(requestId)) {
      ...
      // ignore: avoid_print
      print(
        '[EXT_TOOL_GATE][ABRUPT] requestId=$requestId '
    ...
    // ignore: avoid_print
    print(
      '[RESUME_COORDINATOR][COMPLETE_ATTEMPT] '
      'requestId=$requestId '
  ...
  Future<SessionPersistStatus> persistAiExchangeOnce({
    ...
    if (_persistedExchangeIds.contains(context.requestId)) {
      // ignore: avoid_print
      print(
        '[SESSION_PERSIST][SKIPPED] requestId=${context.requestId} '
    ...
    try {
      ...
      if (!batchResult.ok) {
        ...
        // ignore: avoid_print
        print(
          '[SESSION_PERSIST][FAILED] requestId=${context.requestId} '
      ...
      // ignore: avoid_print
      print(
        '[SESSION_PERSIST][SYNCED] requestId=${context.requestId} '
  ...
  void _upsertLocalSessionIndex({
    ...
    final idx = _localAiSessionIndex.indexWhere(
  ...
  // Deduplicates by sessionId, keeps highest updatedAt, sorts desc, trims ≤20.
  void _mergeIntoSummaries(List<AiSessionSummary> incoming) {
    for (final s in incoming) {
      if (_deletedAiSessionIds.contains(s.sessionId)) {
        continue;
      }
      final existingIdx = _localAiSessionSummaries.indexWhere(
    ...
    // Enforce ≤20 slot bound.
    if (_localAiSessionSummaries.length > 20) {
      _localAiSessionSummaries.removeRange(20, _localAiSessionSummaries.length);
    }
  }
  ...
  // ── AI-RECONSTRUCTION-R18.6W: hidratação reativa V1 + V2 ─────────
  Future<void>? _aiSummaryLoadInFlight;
  String? _aiSummaryLoadUid;
  ...
  Future<void> loadAndMergeAiSessionSummaries(String uid) async {
    ...
    final generation = ++_aiSummaryLoadGeneration;
    ...
    final future = () async {
      try {
        ...
        final canonicalSummaries = <AiSessionSummary>[];
        final legacySummaries = <AiSessionSummary>[];

        if (canonicalResult.isSuccess) {
          final data = canonicalResult.dataOrElse([]);
          canonicalSummaries.addAll(
            data.map(AiSessionSummary.fromCanonicalJson),
          );
        }
  ...
  // ── PLANTAO_QUESTIONS_EXACT_TEN_FINAL_ACCEPTANCE_V1 ────────────────────
  static String _foldPlantaoQuestionsExactTen(String input) {
    return input
        .toLowerCase()
        .replaceAll('á', 'a')
        .replaceAll('à', 'a')
  ...
  static bool _isPlantaoQuestionsExactTenTask(String input) {
    final folded = _foldPlantaoQuestionsExactTen(input);
    return folded.contains('perguntas') || folded.contains('preguntas');
  }

  static int _plantaoQuestionsExactTenCount(String output) {
    return RegExp(r'\?').allMatches(output).length;
  }

  static bool _isValidPlantaoQuestionsExactTen(
    String output, {
    required bool isEs,
  }) {
    final folded = _foldPlantaoQuestionsExactTen(output);
    final expectedHeading = isEs ? 'preguntas clave:' : 'perguntas-chave:';
    if (!folded.contains(expectedHeading)) return false;
    final questionCount = _plantaoQuestionsExactTenCount(output);
    if (questionCount < 6 || questionCount > 10) return false;
    ...
    if (forbidden.any(folded.contains)) return false;
    if (output.contains('📌') ||
        output.contains('💊') ||
        output.contains('🚨')) {
      return false;
  ...
  // MEDCASES_PLANTAO_QUESTIONS_BUTTON_RESILIENT_SPECIFIC_HIDDEN_TRIGGER_V1_B_R0
  // Legacy method name retained to avoid unnecessary topology churn.
  // It no longer exposes a failure sentence. If provider + one repair cannot
  // validate 6-10 safe questions, return six deterministic intake questions.
  ...
  Future<String> _enforcePlantaoQuestionsExactTen({
    ...
    required String candidateOutput,
  }) async {
    ...
    if (_isValidPlantaoQuestionsExactTen(candidateOutput, isEs: isEs)) {
      if (kDebugMode) {
        debugPrint(
          '[PLANTAO_Q10_GATE] requestId=$requestId stage=accept '
          'questions=$initialCount repairAttempted=false',
        );
      }
    ...
    if (kDebugMode) {
      debugPrint(
        '[PLANTAO_Q10_GATE] requestId=$requestId stage=repair_start '
        'questions=$initialCount repairAttempted=true',
      );
    }

    final systemPrompt = isEs
        ? 'REPARADOR DETERMINISTA DE FORMATO — PREGUNTAS CLAVE. '
            'Devuelve únicamente el encabezado "Preguntas clave:" seguido de '
            'EXACTAMENTE 10 preguntas clínicas al paciente, numeradas 1 a 10, '
    ...
    try {
      ...
      if (repaired.success && repaired.text.trim().isNotEmpty) {
        final sanitized = AiSmartRouter.sanitizeAndCheck(
          repaired.text,
          isPlantaoMode: true,
          appLanguage: _lang,
        );
        final repairedText = sanitized.text.trim();
        final repairedCount = _plantaoQuestionsExactTenCount(repairedText);
        if (_isValidPlantaoQuestionsExactTen(repairedText, isEs: isEs)) {
          if (kDebugMode) {
            debugPrint(
              '[PLANTAO_Q10_GATE] requestId=$requestId stage=repair_accept '
              'questions=$repairedCount repairAttempted=true repairSucceeded=true',
            );
          }
          return repairedText;
        }
        ...
        if (kDebugMode) {
          debugPrint(
            '[PLANTAO_Q10_GATE] requestId=$requestId stage=repair_reject '
            'questions=$repairedCount repairAttempted=true repairSucceeded=false',
          );
        }
      }
    } catch (error) {
      if (kDebugMode) {
        debugPrint(
          '[PLANTAO_Q10_GATE] requestId=$requestId stage=repair_exception '
          'repairAttempted=true repairSucceeded=false errorType=${error.runtimeType}',
        );
      }
    }
    ...
    if (kDebugMode) {
      debugPrint(
        '[PLANTAO_Q10_GATE] requestId=$requestId stage=fallback_six '
        'repairAttempted=true repairSucceeded=false fallbackQuestions=6',
      );
    }
    return _plantaoQuestionsExactTenFailClosed(isEs);
  }
  ...
  String _applyPlantaoClinicalRegimenOutputGuard({
    ...
    if (result.modified) {
      // ignore: avoid_print
      print(
        '[PLANTAO_REGIMEN_OUTPUT_GUARD][REPLACED] '
        'requestId=$requestId '
    ...
    if (consistencyGuardedText != result.text) {
      // ignore: avoid_print
      print(
        '[PLANTAO_CLINICAL_RESPONSE_CONSISTENCY_GUARD][MATERIALIZED] '
        'requestId=$requestId '
        'beforeLen=${result.text.length} '
        'afterLen=${consistencyGuardedText.length}',
      );
    }
  ...
  Future<void> _finalizeGptSuccessfulRequest({
    ...
    if (canonicalDecision?.intent == ExternalToolIntent.infusion) {
      ...
      if (!clinicalResult.isValid && clinicalResult.fallback != null) {
        // ignore: avoid_print
        print(
          '[CLINICAL_NUMERIC_GATE][REPLACED] requestId=$requestId '
    ...
    final preRegimenSafeOutput = safeOutput;
    ...
    if (!longResponse && _isPlantaoQuestionsExactTenTask(input)) {
      safeOutput = await _enforcePlantaoQuestionsExactTen(
        requestId: requestId,
        input: input,
        candidateOutput: safeOutput,
      );
    }
    ...
    // permanece agnóstico ao conteúdo visual interno do DTO.
    final ClinicalStructuredOutput? productiveClinicalOutput = clinicalOutput ??
        (!longResponse
    ...
    // Se qualquer barreira local substituir o texto validado, texto e DTO
    ...
    if (kDebugMode && !longResponse) {
      debugPrint(
        '[STRUCTURED_PAID][BOUND] '
        'requestId=$requestId '
        'source=${clinicalOutput != null ? "backend" : "local_adapter"} '
        'dto=${safeClinicalOutput != null}',
      );
    }
    ...
    if (!_completedResolutions.containsKey(requestId)) {
      ...
      final resolvedLink = ExternalToolLinkEngine.build(
        lastUserMessage: visibleUserInput,
      ...
      // ignore: avoid_print
      print(
        '[EXT_TOOL_GATE][GPT_DUPLICATE_SKIPPED] requestId=$requestId '
  ...
  // PHASE3K-C5A-R5: single canonical owner for request/session correlation.
  // This method resolves identifiers only. It does not mutate provider state,
  // call providers, emit callbacks, persist data, or complete a terminal.
  ({String requestId, String sessionId}) _resolveCanonicalAiCorrelation({
    String? requestId,
    String? sessionId,
  }) {
    final normalizedRequestId = requestId?.trim();
    final resolvedRequestId = normalizedRequestId?.isNotEmpty == true
        ? normalizedRequestId!
        : ProviderRouterService.generateRequestId().trim();

    if (resolvedRequestId.isEmpty) {
      throw StateError('AI_REQUEST_ID_EMPTY');
    }

    final normalizedSessionId = sessionId?.trim();
    final normalizedActiveSessionId = _currentConversationSessionId.trim();
    final resolvedSessionId = normalizedSessionId?.isNotEmpty == true
        ? normalizedSessionId!
  ...
  Future<bool> sendAiMessage(
    ...
    PlantaoContinuationType shadowContinuationType =
    ...
    final phase3kQaAllowlistConfigured =
        _plantaoQaCutoverSupport.hasConfiguredUidAllowlist;
    final phase3kQaEligible = _plantaoQaCutoverSupport.isEligible(
      isPlantao: phase3kQaIsPlantao,
      authenticatedUid: phase3kQaAuthenticatedUid,
    );
    final phase3kActiveCutoverController = phase3kQaEligible
        ? PlantaoBufferedCutoverController(
            enabled: true,
            pipeline: AppProviderAiResponsePipeline.fromAppProvider(
              this,
              providerLabel: 'app_provider_qa_allowlist',
            ),
          )
        : _plantaoBufferedCutoverController;

    final phase3kBufferedCutoverBusy = phase3kActiveCutoverController.enabled &&
        _plantaoBufferedCutoverExecutionActive &&
        !longResponse;

    // Phase3K-C5A-R5: resolve the canonical pair before cutover.
    // Phase3K-C5A-R8: resolve it even when another cutover is
    // still unwinding. This keeps observability correlated and allows a
    // fail-closed busy rejection instead of a silent legacy bypass.
    if (phase3kActiveCutoverController.enabled && !longResponse) {
      final phase3kCorrelation = _resolveCanonicalAiCorrelation(
        requestId: phase3kResolvedRequestId,
        sessionId: phase3kResolvedSessionId,
      );
      phase3kResolvedRequestId = phase3kCorrelation.requestId;
      phase3kResolvedSessionId = phase3kCorrelation.sessionId;
    }
    // Phase3K-C5A-R5: the same pair continues through pipeline or legacy.

    if (phase3kQaAllowlistConfigured) {
      final phase3kQaEligibilityReason = !phase3kQaIsPlantao
          ? PlantaoQaCutoverReason.notPlantaoMode
          : phase3kQaAuthenticatedUid == null ||
                  phase3kQaAuthenticatedUid.isEmpty
              ? PlantaoQaCutoverReason.unauthenticatedUid
              : phase3kQaEligible
                  ? PlantaoQaCutoverReason.uidAllowlisted
                  : PlantaoQaCutoverReason.uidNotAllowlisted;

      if (phase3kQaEligible && phase3kBufferedCutoverBusy) {
        _plantaoQaCutoverSupport.emit(
          event: PlantaoQaCutoverEvent.executionRejectedWhileActive,
          reason: PlantaoQaCutoverReason.cutoverAlreadyActive,
          requestId: phase3kResolvedRequestId,
          sessionId: phase3kResolvedSessionId,
        );
      } else {
        _plantaoQaCutoverSupport.emit(
          event: phase3kQaEligible
              ...
              : PlantaoQaCutoverEvent.eligibilityRejected,
          reason: phase3kQaEligibilityReason,
          requestId: phase3kResolvedRequestId,
          sessionId: phase3kResolvedSessionId,
        );
      }
    ...
    final phase3kShouldAttemptBufferedCutover =
        phase3kActiveCutoverController.enabled &&
            !_plantaoBufferedCutoverExecutionActive &&
            !longResponse &&
            phase3kResolvedRequestId != null &&
            phase3kResolvedRequestId.isNotEmpty &&
            phase3kResolvedSessionId != null &&
            phase3kResolvedSessionId.isNotEmpty;

    if (phase3kShouldAttemptBufferedCutover) {
      ...
      final phase3kRequest = AiRequestContract(
        requestId: phase3kResolvedRequestId,
        sessionId: phase3kResolvedSessionId,
        input: input,
        mode: AiRequestMode.plantao,
        locale: _lang == 'es' ? AiRequestLocale.es : AiRequestLocale.pt,
        sourceSurface: AiSourceSurface.other,
        metadata: const <String, Object?>{
          'cutover': 'phase3k_qa_allowlist_default_empty',
        },
      );

      _plantaoBufferedCutoverExecutionActive = true;
      late final PlantaoBufferedCutoverDecision phase3kDecision;
      try {
        phase3kDecision = await phase3kActiveCutoverController.execute(
          phase3kRequest,
      ...
      switch (phase3kDecision.disposition) {
        case PlantaoBufferedCutoverDisposition.committed:
          if (phase3kQaEligible) {
            _plantaoQaCutoverSupport.emit(
              ...
              reason: PlantaoQaCutoverReason.commitValidated,
              requestId: phase3kResolvedRequestId,
              sessionId: phase3kResolvedSessionId,
            );
            _plantaoQaCutoverSupport.emit(
          ...
          // PHASE3K-C5A-R9C: a UI finaliza a bolha e libera
          // _isStreaming dentro de onDone. O DTO só pode ser associado
          // depois que essa bolha final e seu texto estiverem commitados.
          assert(() {
            debugPrint(
        ...
        case PlantaoBufferedCutoverDisposition.rejectedAfterStart:
          if (phase3kQaEligible) {
            _plantaoQaCutoverSupport.emit(
              event: PlantaoQaCutoverEvent.validationRejected,
              reason: PlantaoQaCutoverReason.validationRejected,
              requestId: phase3kResolvedRequestId,
              sessionId: phase3kResolvedSessionId,
            );
          }
        ...
        case PlantaoBufferedCutoverDisposition.bypassed:
          break;

        case PlantaoBufferedCutoverDisposition.fallbackAllowed:
          if (phase3kQaEligible) {
            _plantaoQaCutoverSupport.emit(
              event: PlantaoQaCutoverEvent.legacyFallbackBeforeEvent,
              reason: PlantaoQaCutoverReason.fallbackBeforeFirstEvent,
              requestId: phase3kResolvedRequestId,
              sessionId: phase3kResolvedSessionId,
            );
          }
    ...
    return _sendAiMessageLegacyCore(
      ...
      shadowContinuationType: shadowContinuationType,
      shadowRequestedSections: shadowRequestedSections,
      phase3kEffectPolicy: AiProviderEffectPolicy.legacy,
    );
  }

  /// Internal bridge used only by the typed AI pipeline.
  /// It bypasses the public selector and cannot recurse.
  Future<bool> sendAiMessageForPipeline(
    ...
    // PHASE3K-C5A-R8-V2: aliases preserve the bridge contract that this
    // method forwards internal callbacks without invoking the public
    // parameter identifiers directly.
    final bufferedOnChunk = onChunk;
    final bufferedOnDone = onDone;
    final bufferedOnStructuredDone = onStructuredDone;
    final bufferedOnError = onError;

    final cancellationRequestId = pipelineRequestId?.trim();
    if (cancellationRequestId == null || cancellationRequestId.isEmpty) {
      bufferedOnError.call('PIPELINE_REQUEST_ID_REQUIRED');
      return false;
    }

    if (_plantaoBufferedPipelineCancellation != null) {
      bufferedOnError.call('PIPELINE_CANCELLATION_OWNER_BUSY');
      return false;
    }

    void clearCancellationOwner() {
      if (_plantaoBufferedPipelineCancellationRequestId !=
    ...
    void forwardBufferedCancellation() {
      clearCancellationOwner();
      bufferedOnError.call('PIPELINE_STREAM_CANCELLED');
    }

    _plantaoBufferedPipelineCancellationRequestId = cancellationRequestId;
    _plantaoBufferedPipelineCancellation = forwardBufferedCancellation;

    try {
      final accepted = await _sendAiMessageLegacyCore(
        input,
        onChunk: bufferedOnChunk,
        onDone: (finalText) {
          clearCancellationOwner();
          bufferedOnDone.call(finalText);
        },
        onStructuredDone: bufferedOnStructuredDone == null
            ? null
  ...
  Future<bool> _sendAiMessageLegacyCore(
    ...
    // `input` remains the complete clinical model payload.
    // History and canonical persistence use only visible text.
    final visibleInputCandidate = visibleUserInput?.trim() ?? '';
    final persistedUserInput =
        visibleInputCandidate.isNotEmpty ? visibleInputCandidate : input;
    ...
    final normalizedPipelineRequestId = pipelineRequestId?.trim();
    ...
    void guardiaTraceOnChunk(String accumulated) {
      guardiaTraceProviderChunkIndex++;
      guardiaTraceProviderAccumulatedLen = accumulated.length;
      assert(() {
        if (!longResponse &&
            (guardiaTraceProviderChunkIndex <= 3 ||
                guardiaTraceProviderChunkIndex % 25 == 0)) {
          debugPrint(
            '[GUARDIA_TRACE] stage=I1_provider_chunk '
            'trace=$thisRequestId '
            'chunkIndex=$guardiaTraceProviderChunkIndex '
            'accumulatedLen=$guardiaTraceProviderAccumulatedLen',
          );
        }
        return true;
      }());
    ...
    // PHASE3K-C5A-R5: reuse the same canonical request/session owner used
    // before cutover. Legacy-only calls still resolve through this owner.
    final phase3kHadActiveSession =
        _currentConversationSessionId.trim().isNotEmpty;
    final phase3kLegacyCorrelation = _resolveCanonicalAiCorrelation(
      requestId: thisRequestId,
      sessionId: pipelineSessionId,
    );
    _currentConversationSessionId = phase3kLegacyCorrelation.sessionId;
    if (!phase3kHadActiveSession) {
      // ignore: avoid_print
      print(
        '[AI_SESSION_INDEX][NEW_SESSION] '
    ...
    // rendering or persistence. Failure is observational and cannot block the
    // productive legacy request.
    if (longResponse) {
      _lastPlantaoShadowRequest = null;
      _lastPlantaoFinalizationShadow = null;
      _lastPlantaoValidationShadow = null;
      _lastPlantaoPersistenceShadow = null;
      _lastPlantaoRetrievalShadow = null;
      _lastPlantaoDrugEvidenceShadow = null;
      _plantaoDrugEvidenceRequestFuture = null;
      _lastPlantaoDrugEvidenceRequestShadow = null;
      _lastPlantaoDrugEvidenceFinalizationJoinShadow = null;
      _lastPlantaoDrugIdentityProvenanceBindingShadow = null;
      _lastPlantaoDrugProvenancePersistencePayloadShadow = null;
      _lastPlantaoDrugProvenancePersistencePayloadParityShadow = null;
      _lastPlantaoDrugProvenanceSemanticEqualityShadow = null;
      _lastPlantaoDrugProvenancePersistenceReviewEnvelopeShadow = null;
      _lastPlantaoDrugPersistenceCutoverBlockerAuditShadow = null;
      _lastPlantaoDrugTypedRegimenCapabilityMatrixShadow = null;
      _lastPlantaoDrugExplicitTypedRegimenEvidenceProjectionShadow = null;
      _lastPlantaoDrugProjectedEvidenceBundleOverlayShadow = null;
      _lastPlantaoDrugProjectedEvidenceValidationReplayComparisonShadow = null;
    } else {
      _lastPlantaoFinalizationShadow = null;
      _lastPlantaoValidationShadow = null;
      _lastPlantaoPersistenceShadow = null;
      ...
      _lastPlantaoDrugProjectedEvidenceValidationReplayComparisonShadow = null;
      try {
        _lastPlantaoShadowRequest = PlantaoRequestShadowAdapter.build(
          requestId: activeSessionCtx.requestId,
          sessionId: activeSessionCtx.sessionId,
          question: input,
          languageCode: activeSessionCtx.locale,
          fromButton: fromButton,
          continuationType: shadowContinuationType,
          requestedSections: shadowRequestedSections,
          clientContext: const <String, Object?>{
            'phase': '3C',
            'shadowMode': true,
            'productivePipelineConnected': false,
          },
        );
        assert(() {
          final shadow = _lastPlantaoShadowRequest!;
          debugPrint(
            '[PLANTAO_SHADOW_REQUEST] requestId=${shadow.requestId} '
            'sessionIdHash=${shadow.sessionId.hashCode} '
            'trigger=${shadow.trigger.wireName} '
            'continuationType=${shadow.continuationType.name} '
            'sections=${shadow.requestedSections.map((item) => item.name).join(",")}',
          );
          return true;
        }());
        _capturePlantaoDrugEvidenceShadow(
          request: _lastPlantaoShadowRequest!,
          originalUserInput: input,
        );
      } catch (error, stackTrace) {
        _lastPlantaoShadowRequest = null;
        debugPrint(
          '[PLANTAO_SHADOW_REQUEST][INVALID] requestId=$thisRequestId '
          'error=$error stack=$stackTrace',
        );
      }
    }
    ...
    AppResumeCoordinator.instance.registerAiRequest(
      ...
      onTimeout: () {
        debugPrint(
          '[AI_RESUME][BUILD320] requestId=$thisRequestId '
        ...
        aiChatProvider.setStreaming(
          false,
        ); // BUILD 326.1: resume timeout — UI desbloqueia
        _aiStreamSub?.cancel();
        _aiStreamSub = null;
        _aiCallInFlight = false;
        debugPrint(
    ...
    try {
      ...
      void Function(String, [ClinicalStructuredOutput?]) wrappedOnDone =
        ...
        assert(() {
          if (!longResponse) {
            debugPrint(
              '[GUARDIA_TRACE] stage=I3_provider_final '
              'tsUs=${DateTime.now().microsecondsSinceEpoch} '
              'trace=$thisRequestId '
              'chunkIndex=$guardiaTraceProviderChunkIndex '
              'accumulatedLen=$guardiaTraceProviderAccumulatedLen '
        ...
        if (_wrapperFired) {
          debugPrint(
            '[BUILD318][DEDUP] wrappedOnDone dropped — already fired '
        ...
        // callback has completed. This Future is observational, never awaited by
        // the productive terminal path and never triggers notifyListeners.
        final shadowRequest = _lastPlantaoShadowRequest;
        if (!longResponse &&
            shadowRequest != null &&
            shadowRequest.requestId == thisRequestId) {
          _plantaoShadowExecutionIsolator.launch(
            () => _capturePlantaoFinalizationShadow(
            ...
            onFailure: (error, stackTrace) {
              debugPrint(
                '[PLANTAO_SHADOW_FINALIZATION][FAILED] '
                'requestId=$thisRequestId error=$error stack=$stackTrace',
              );
            },
          );
        }
      ...
      final wrappedOnError = (String err) {
        if (_wrapperFired) {
          debugPrint(
            '[BUILD318][DEDUP] wrappedOnError dropped — already fired '
      ...
      // ── M14-R7: decisão única de provider antes do gate GPT SSE ─────────────
      final contractName =
      ...
      // ══════════════════════════════════════════════════════════════════════════
      if (shouldUseGptSse) {
        void finishGptSsePreflightError(String message) {
          wrappedOnError(message);
          ExternalToolLinkEngine.releaseCanonicalDecision(
            requestId: thisRequestId,
            decision: canonicalDecision,
          );
        ...
        // ignore: avoid_print
        print(
          '[AI_E2E][T0] requestId=$thisRequestId attempt=2 '
        ...
        // ignore: avoid_print
        print(
          '[AI_E2E][PROVIDER_SWITCH] requestId=$thisRequestId '
        ...
        if (kIsWeb) {
          try {
            ...
            debugPrint(
              '[AI_E2E][AUTH_ERROR] requestId=$thisRequestId '
              'error=token_fetch_failed detail=$e',
            );
            finishGptSsePreflightError(
              _lang == 'es'
        ...
        if (fromButton &&
          ...
          _threadManager.primeFromHistory(
            List<Map<String, String>>.from(_sanitizedHistory),
          );
          if (kDebugMode) {
            debugPrint(
              '[PHASE3I_J2B2][CONTINUATION_REHYDRATED][QA] '
              'requestId=$thisRequestId historyLen=${_sanitizedHistory.length}',
            );
          }
        }
        ...
        if (qaThreadStatus.action == ThreadAction.newThread && !longResponse) {
          ...
          debugPrint(
            '[AI_E2E][HISTORY_RESET] requestId=$thisRequestId '
        ...
        bool qaFallbackStarted = false;
        int qaDeltaCount = 0;
        Timer? qaTimer;

        Future<void> runGeminiPaidFallbackFromGptSse(String reason) async {
          if (qaFallbackStarted) return;
          qaFallbackStarted = true;
          qaCompFired = true;
          qaTimer?.cancel();

          final activeSubscription = _gptStreamSub;
          _gptStreamSub = null;
          if (activeSubscription != null) {
            // Não aguarda a própria subscription dentro de onData/onError/onDone.
            // O fechamento HTTP imediato ocorre na chamada cancel() logo abaixo.
          ...
          print(
            '[AI_E2E][PROVIDER_SWITCH] requestId=$thisRequestId '
            'fromProvider=gpt_4o_mini toProvider=gemini_paid '
            'reason=$reason partialLen=${qaAccumulator.length}',
          );
          ...
          try {
            ...
            if (!paidResult.success || paidResult.text.trim().isEmpty) {
              throw AiSafeOutputException(
                message: paidResult.errorCode ?? 'gemini_paid_failed',
                requestId: thisRequestId,
              );
            }

            final responseProcessor =
                AiResponseFinalizationProcessor.withExistingImplementations();

            try {
              final finalizationOutcome = await responseProcessor.process(
                snapshot: FinalOutputSnapshot(
                  rawOutput: paidResult.text.trim(),
                  sessionId: activeSessionCtx.sessionId,
              ...
              final truncationInspection =
                  finalizationOutcome.truncation?.inspection;
              if (truncationInspection != null) {
                TruncationInspector.emitTelemetry(
                  requestId: thisRequestId,
                  result: truncationInspection,
                );
              }

              if (!finalizationOutcome.isReady) {
                throw AiSafeOutputException(
                  message: finalizationOutcome.failureReason ??
                      finalizationOutcome.failureCode ??
                      'gemini_paid_finalization_failed',
                  requestId: thisRequestId,
                );
              }

              final finalizationResult = finalizationOutcome.result!;
              ...
              responseProcessor.release(thisRequestId);
            }
          } on AiSafeOutputException catch (error) {
            _aiStreamActive = false;
            aiChatProvider.setStreaming(false);
            wrappedOnError(
              _lang == 'es'
            ...
            // ignore: avoid_print
            print(
              '[AI_E2E][GEMINI_PAID_FAILED] '
              'requestId=$thisRequestId reason=${error.message}',
            );
          } catch (error) {
            _aiStreamActive = false;
            aiChatProvider.setStreaming(false);
            wrappedOnError(
              _lang == 'es'
                  ? 'Error temporal en el asistente IA. Intenta nuevamente. ⚕'
                  : 'Erro temporário no assistente IA. Tente novamente. ⚕',
            );
            ExternalToolLinkEngine.releaseCanonicalDecision(
              requestId: thisRequestId,
              decision: canonicalDecision,
            ...
            // ignore: avoid_print
            print(
              '[AI_E2E][GEMINI_PAID_EXCEPTION] '
              'requestId=$thisRequestId error=$error',
            );
          }
        }
        ...
        qaTimer = Timer(Duration(milliseconds: qaTimeoutMs), () {
          ...
          qaCompFired = true;
          debugPrint(
            '[AI_E2E][TIMEOUT] requestId=$thisRequestId '
            'timeoutMs=$qaTimeoutMs elapsedMs='
            '${DateTime.now().millisecondsSinceEpoch - globalStartMs}',
          );
          unawaited(runGeminiPaidFallbackFromGptSse('gpt_sse_timeout'));
        });
        ...
        _gptStreamSub = qaStream.listen(
          (AiEvent event) async {
            ...
            if (qaFallbackStarted) {
              debugPrint(
                '[AI_E2E][POST_FALLBACK_DROP] '
                'requestId=$thisRequestId event=${event.runtimeType}',
              );
              return;
            }
            ...
            switch (event) {
              ...
              case AiStarted e:
                // ignore: avoid_print
                print(
                  '[AI_E2E][STARTED] requestId=$thisRequestId '
              ...
              case AiTextDelta e:
                ...
                guardiaTraceOnChunk(qaAccumulator.toString());
              ...
              case AiCompleted e:
                ...
                // ignore: avoid_print
                print(
                  '[AI_E2E][T2_TRANSPORT_DONE] requestId=$thisRequestId '
                ...
                // ignore: avoid_print
                print(
                  '[AI_E2E][SANITIZED] requestId=$thisRequestId '
                  'rawLen=${qaRawText.length} mode=${longResponse ? "estudo" : "plantao"}',
                );

                // AI-RECONSTRUCTION-R18.6G-R2:
                // Substitui somente inspeção, reparo, sanitização
                // e parsing estrutural nesta rota GPT SSE.
                //
                // Validação numérica, persistência, ferramentas,
                // UI, release canônico e conclusão permanecem externos.
                final responseProcessor = AiResponseFinalizationProcessor
                    .withExistingImplementations();
                ...
                try {
                  final finalizationOutcome = await responseProcessor.process(
                    ...
                    locale:
                        _lang == 'es' ? AiRequestLocale.es : AiRequestLocale.pt,
                  ...
                  final qaFinalizationResult = finalizationOutcome.result!;

                  final qaFinalText = _applyPlantaoClinicalRegimenOutputGuard(
                    userInput: input,
                  ...
                  // ignore: avoid_print
                  print(
                    '[AI_E2E][COMPLETED] requestId=$thisRequestId '
                  ...
                  // ignore: avoid_print
                  print(
                    '[AI_FINALIZATION] GPT_SSE_REJECTED '
                    'requestId=${safeError.requestId} '
                    'reason=${safeError.message} '
                    'fallback=gemini_paid',
                  );
                  await runGeminiPaidFallbackFromGptSse(
                    'gpt_sse_finalization_failed',
                  );
                  return;
                } finally {
                  responseProcessor.release(thisRequestId);
              ...
              case AiFailed e:
                ...
                if (e.code == 'gpt_sse_budget_guard') {
                  _gptStreamSub = null;
                  _activeGptClient = null;
                  _aiStreamActive = false;
                  aiChatProvider.setStreaming(false);
                ...
                // ignore: avoid_print
                print(
                  '[AI_E2E][GPT_FAILED_FALLBACK] '
                  'requestId=$thisRequestId code=${e.code} '
                  'retryable=${e.retryable} '
                  'partialLen=${e.partialText?.length ?? 0}',
                );
                await runGeminiPaidFallbackFromGptSse('gpt_sse_${e.code}');
                return;
              ...
              case AiStreamReset e:
                // ignore: avoid_print
                print(
                  '[AI_E2E][STREAM_RESET] requestId=$thisRequestId '
          ...
          onError: (Object error) async {
            ...
            qaTimer?.cancel();
            debugPrint(
              '[AI_E2E][STREAM_EXCEPTION] '
              'requestId=$thisRequestId error=$error '
              'fallback=gemini_paid',
            );
            await runGeminiPaidFallbackFromGptSse('gpt_sse_stream_exception');
          },
          onDone: () async {
            ...
            if (qaCompFired) {
              ...
              return;
            } // EOF sem conclusão: o parcial continua provisório e o Gemini Paid
            // assume sem repetir GPT nem concatenar tentativas.
            qaCompFired = true;
            qaTimer?.cancel();
            debugPrint(
              '[AI_E2E][EOF_NO_COMPLETION] '
              'requestId=$thisRequestId '
              'accumulatorLen=${qaAccumulator.length} '
              'fallback=gemini_paid',
            );
            await runGeminiPaidFallbackFromGptSse('gpt_sse_eof_no_completion');
          },
          cancelOnError: false,
        );
      ...
      // ignore: avoid_print
      print(
        '[ORDEM49_MODE_SYNC] requestId=$thisRequestId '
      ...
      // M14-R7: contractName/aiPriority foram resolvidos uma única vez antes
      // do gate GPT SSE. O fluxo normal reutiliza exatamente a mesma decisão.
      ...
      Future<bool> tryPaidFallback(String reason) async {
        ...
        if (_openAiKey.isNotEmpty) {
          ...
          if (gptResult.success && gptResult.text.isNotEmpty) {
            // ignore: avoid_print
            print(
              '[RAW_AI_OUTPUT][GPT_PROXY] len=${gptResult.text.length} '
            ...
            if (!_isFallbackText(gptText)) {
              ...
              if (gptText.isNotEmpty) {
                final gptPaidPersistStatus = await persistAiExchangeOnce(
                ...
                print(
                  ...
                  'status=${gptPaidPersistStatus.runtimeType}',
                );

                if (gptPaidPersistStatus is SessionPersistSynced) {
        ...
        if (paidResult.success && paidResult.text.isNotEmpty) {
          ...
          // ignore: avoid_print
          print(
            '[RAW_AI_OUTPUT][PAID_PROXY] len=${paidResult.text.length} '
            'requestId=$requestId mode=${longResponse ? "estudo" : "plantao"}',
          );
          ...
          if (!_isFallbackText(paidText)) {
            ...
            if (paidText.isNotEmpty) {
              final geminiPaidPersistStatus = await persistAiExchangeOnce(
                context: activeSessionCtx,
                userInput: persistedUserInput,
                userDisplayText: persistedUserDisplayText,
              ...
              print(
                ...
                'status=${geminiPaidPersistStatus.runtimeType}',
              );

              if (geminiPaidPersistStatus is SessionPersistSynced) {
                _freeStreamTxn.emitPersistTelemetry(
                  sessionId: activeSessionCtx.sessionId,
                );
              } else {
      ...
      // ── M13-R2: ROTEAMENTO POR MODO ──────────────────────────────────────────
      // Guardia/Plantão com prioridade critical permanece no caminho pago direto:
      ...
      if (forcePaidCanary) {
        ...
        debugPrint(
          '[ROUTER_BLINDAGEM] geminiConnected=true '
        ...
        unawaited(() async {
          ...
          if (paidResult.success && paidResult.text.isNotEmpty) {
            ...
            if (!_isFallbackText(paidText)) {
              ...
              if (paidText.isNotEmpty) {
                final criticalPaidPersistStatus = await persistAiExchangeOnce(
                  context: activeSessionCtx,
                  userInput: persistedUserInput,
                  userDisplayText: persistedUserDisplayText,
                  assistantOutput: paidText,
                ...
                print(
                  ...
                  'status=${criticalPaidPersistStatus.runtimeType}',
                );

                if (criticalPaidPersistStatus is SessionPersistSynced) {
                  _freeStreamTxn.emitPersistTelemetry(
                    sessionId: activeSessionCtx.sessionId,
                  );
                } else {
      ...
      if (_canonicalTaskOverride.isNotEmpty) {
        // ignore: avoid_print
        print(
          '[CANONICAL_TASK_ENFORCEMENT] requestId=$thisRequestId '
      ...
      _aiStreamSub = stream.listen(
        (chunk) async {
          ...
          if (chunk.isDone && !chunk.isError) {
            ...
            // uma resposta concluída. Deixa stream_onDone assumir e iniciar o
            // mecanismo de auto-retry já existente.
            if (accumulator.toString().trim().isEmpty) {
              if (kDebugMode) {
                debugPrint(
                  '[EMPTY_TERMINAL_DEFER] source=chunk_isDone '
                  'action=wait_stream_onDone',
                );
              }
              return;
            }
            ...
            // ignore: avoid_print
            print(
              '[RAW_AI_OUTPUT][FREE_STREAM] len=${rawText.length} '
              'requestId=$requestId mode=${longResponse ? "estudo" : "plantao"}',
            );
            ...
            try {
              ...
              if (finalText.isNotEmpty && !_isFallbackText(finalText)) {
                ...
                // uid, sessionId e requestId vêm do ActiveAiSessionContext.
                ...
                print(
                  ...
                  'requestId=$thisRequestId '
                  'sessionId=${activeSessionCtx.sessionId} '
                  'route=gemini_free_chunk_isDone '
                  'status=${freePersistStatus.runtimeType}',
                );

                // A transação só registra assistantPersisted após sync real.
                if (freePersistStatus is SessionPersistSynced) {
                  _freeStreamTxn.emitPersistTelemetry(
                    sessionId: activeSessionCtx.sessionId,
                  );
                } else {
              ...
              if (!_freeStreamTxn.dropIfBusinessEventTerminal(
                ...
                if (_freeStreamTxn.tryMarkToolResolutionStarted()) {
                  ...
                  final resolvedLink = ExternalToolLinkEngine.build(
                    lastUserMessage: persistedUserInput,
                  ...
                  // ignore: avoid_print
                  print(
                    '[EXT_TOOL_GATE][DUPLICATE_DROPPED] '
                    'requestId=${_freeStreamTxn.parentRequestId} '
                    'callSite=canonical_finalizer',
                  );
                }
              ...
              final String freeUiText = finalText.isNotEmpty
                  ? finalText
                  : _lang == 'es' // BUILD 254
              ...
              final ClinicalStructuredOutput? freeClinicalOutput =
                  !longResponse && finalText.isNotEmpty
                      ? PlantaoLocalClinicalOutputAdapter.fromValidatedText(
                          finalText,
                        )
                      : null;
              if (kDebugMode) {
                debugPrint(
                  '[STRUCTURED_FREE][DERIVED] '
                  'requestId=$thisRequestId '
                  'source=chunk_isDone '
                  'prescriptionCount='
                  '${freeClinicalOutput?.prescricao.length ?? 0}',
                );
              }
        ...
        onDone: () {
          ...
          if (finalText.isNotEmpty && !_isFallbackText(finalText)) {
            ...
            if (_freeStreamRetryCount < 1 &&
              ...
              if (kDebugMode) {
                debugPrint(
                  '[BUILD432][AUTO_RETRY] stream empty → retry '
                ...
                _aiStreamSub = retryStream.listen(
                  (chunk) async {
                    if (!chunk.isError && chunk.text.isNotEmpty) {
                      accumulator.write(chunk.text);
                      guardiaTraceOnChunk(accumulator.toString());
                    }
                    if (chunk.isDone && !chunk.isError) {
                      // MICRO-BUILD 462E-A.5.3.7: transaction ownership gate for retry stream.
                      // STREAM-ZERO-I.1: o segundo evento terminal vazio
                      // deve deixar retry_onDone executar a escalada existente.
                      if (accumulator.toString().trim().isEmpty) {
                        if (kDebugMode) {
                          debugPrint(
                            '[EMPTY_TERMINAL_DEFER] '
                            'source=retry_chunk_isDone '
                            'action=wait_retry_onDone',
                          );
                        }
                        return;
                      }
                      ...
                      if (retryTruncCheck.isTruncated &&
                        ...
                        // ignore: avoid_print
                        print(
                          '[TRUNCATION_CHECK] BARRIER_TRIGGERED '
                          'requestId=$thisRequestId '
                      ...
                      retryFinalText = _applyPlantaoClinicalRegimenOutputGuard(
                        userInput: input,
                      ...
                      if (retryFinalText.isNotEmpty &&
                        ...
                        final retryPersistStatus = await persistAiExchangeOnce(
                          context: activeSessionCtx,
                          userInput: persistedUserInput,
                          userDisplayText: persistedUserDisplayText,
                          assistantOutput: retryFinalText,
                        ...
                        print(
                          ...
                          'requestId=$thisRequestId '
                          'sessionId=${activeSessionCtx.sessionId} '
                          'route=gemini_free_retry_chunk_isDone '
                          'status=${retryPersistStatus.runtimeType}',
                        );

                        // assistantPersisted somente após sincronização real.
                        if (retryPersistStatus is SessionPersistSynced) {
                          _freeStreamTxn.emitPersistTelemetry(
                            sessionId: activeSessionCtx.sessionId,
                          );
                        } else {
                        ...
                        _aiStreamSub = null;
                        // MICRO-BUILD 462E-A.5.3.4+R5: wrappedOnDone → release → completeAiRequest LAST.
                        final ClinicalStructuredOutput? retryClinicalOutput =
                            !longResponse && retryFinalText.isNotEmpty
                                ? PlantaoLocalClinicalOutputAdapter
                                    .fromValidatedText(
                                    retryFinalText,
                                  )
                                : null;
                        if (kDebugMode) {
                          debugPrint(
                            ...
                            'source=retry_chunk_isDone '
                            'prescriptionCount='
                            '${retryClinicalOutput?.prescricao.length ?? 0}',
                          );
                        }
                        ...
                        _aiStreamSub = null;
                        debugPrint(
                          '[BUILD432][AUTO_RETRY] retry also empty → '
                        ...
                        unawaited(() async {
                          final handled = await tryPaidFallback(
                            'empty_stream_after_retry',
                          );
                          if (!handled) {
                            wrappedOnError(
                              _lang == 'es'
              ...
              if (kDebugMode) {
                debugPrint(
                  '[AI_ROUTER] stream closed empty (retry esgotado) → '
  ...
  Future<String> _buildAIAnswerImpl(String input) async {
    ...
    print(
      '[SYSTEM_PROMPT_AUDIT][buildAIAnswer] '
      ...
      if (winner == null) {
        ...
        if (looksLikeClinical && queryTerms.isNotEmpty) {
          ...
          if (_patient.age.isNotEmpty) {
            buf2f.writeln(
              'paciente="${_patient.age} anos, ${_patient.sex}'
